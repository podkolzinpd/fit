...
- Today, date picker, переключатель периода и week arrows используют compact
  44 px. По умолчанию сохраняется недельная neutral grid: пары `Пн/Вт`,
  `Ср/Чт`, `Пт/Сб` и полноширинное воскресенье. Парные карточки имеют общий
  минимум на пять строк и растут вместе по более заполненному дню; внутреннего
  скролла и скрытого остатка нет.
- Режим `2 недели` использует семь колонок и две календарные строки. Все
  тренировки остаются видимыми без `ещё N` и вложенного скролла; каждая строка
  недели растёт по самому заполненному дню. На мобильном время и имя получают
  только компактное отображение, а полные значения сохраняются в доступной
  подписи и подробном дне.
...
- Empty week сохраняет семь интерактивных карточек, а empty `2 недели` — 14
  интерактивных дней с явным пустым состоянием; empty day — реальная часовая
  сетка без искусственной empty-card. Untimed, loading/error/retry, internal day
  scroll и date navigation сохраняют существующую продуктовую логику. Выбранный
  диапазон загружается целиком без пагинации.
