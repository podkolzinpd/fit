...
Обновлено: 2026-09-08. База: `main` `f9d0d0e7`. Активно: определить безопасную причину отклонения tenant dry-run на stage.
...
- Две локальные репетиции перенесли 35 synthetic строк во всех 28 таблицах; dry-run откатился, повторный apply вставил 0 строк.
- YAFIT-478: полный check — 1243 frontend и 346 API тестов; local verify — 988 SQL/RLS и 30 actor/RLS; visual/e2e — 390/430, dark, trainer 1440.
- Автовыбор tenant: 349 API и 110 infra policy tests прошли; автоматический apply запрещён.
- Remote audit `34213523653` выбрал изолированный cohort: 1 клиент, 5 строк. Dry-run `34215055534` дошёл до private stage runner, вернул `409` и ничего не сохранил.
## Ближайший порядок
1. Влить безопасную диагностику target rejection code и повторить dry-run.
2. Исправить точную причину `409`, повторить dry-run и только затем отдельно подтвердить stage apply.
