...
Проверенный базовый `main`: `0d6cbbd` (`YAFIT-381: сделать выбранный период Progress нейтральнее (#615)`)

## Активное изменение

- Ассистент тренера сохраняет уже продиктованные упражнения, если перед
  разбором тренировки требуется уточнить клиента. После выбора клиента
  продолжается тот же черновик без повторной диктовки.
