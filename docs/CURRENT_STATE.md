...
Проверенный базовый `main`: `c5b1405` (`fix(yandex): smoke exact progress fixture (#636)`)

## Активное изменение

- После успешной привязки клиент и тренер получают отдельный понятный экран
  результата и переходят дальше только явной кнопкой. Ошибка подключения при
  другом основном тренере объясняет необходимость сначала завершить текущую
  связь. Серверная логика привязки и данные в этом UI-изменении не меняются.
- UI отключения и смены тренера остаются следующими отдельными задачами.
