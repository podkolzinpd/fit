...
- `YAFIT-329` стабилизирует голосовой и текстовый разбор без изменения
  SpeechKit: порядок веса и схемы подходов больше не влияет на результат,
  однозначная связка «сведение и разведение ног» становится двумя упражнениями,
  а безопасно найденные локально значения дополняют ответ LLM или переживают
  его временную сетевую ошибку.
- `YAFIT-319` убирает пустую проверку перед ручным выбором: каталог направления
  открывается сразу, после выбора появляется заполненная проверка, а закрытие
  без выбора возвращает к вводу. Беговые форматы и голосовой разбор не меняются.
- `YAFIT-320` уплотняет единый каталог упражнений во всех сценариях: поиск
  сворачивает фильтры без сброса значений, открытие фильтров убирает клавиатуру,
  активные фильтры считаются на кнопке, а список и служебные действия занимают
  меньше места. Беговые форматы, интервалы, СБУ и множественный выбор сохранены.
