...
Проверенный базовый `main`: `6fcdb66` (`feat(yandex): add app feedback stage slice (#711)`)
...
- «Следующий шаг» — отдельный компактный черновик после результатов. Код
  ...
  совпадении смысла и чисел. Подтверждение, замена и отказ ничего не сохраняют;
  переход появляется после подтверждения.
...
- Этап 9 YAFIT-414 после rebase: check подтверждён — 981 frontend, 238 API,
  66 infra; DB/RLS 886, actor API/RLS 24; browser/WebKit/visual — зелёные.

## Ближайший порядок
1. Завершить этап 9: PR, CI, merge, production smoke и закрытие roadmap YAFIT-414.
2. Завершить delivery `app_feedback`, затем продолжить Yandex parity/cutover.
## Отложено
