...
Проверенный базовый `main`: `fd1afde` (`YAFIT-379: упростить визуальную иерархию Live (#613)`)

## Активное изменение

- На собственной незавершённой тренировке клиента меню копирования и удаления
  находится в общей шапке рядом со статусом. Нижняя зона содержит только
  вторичное редактирование; завершённые тренировки и интерфейс тренера не
  изменены.
