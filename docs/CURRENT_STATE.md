...
- В незавершённой ветке `codex/assistant-program-quality` остаётся улучшение
  состава программ. До cutover его нужно либо слить с parity обоих backend,
  либо явно исключить из релиза.
...
- #1027 восстановил явный ввод кодов приглашения; #1028/#1030 оставили готовые
  тренировки постоянно доступными на отдельной вкладке; #1029 разрешил
  активному клиенту менять профиль через эквивалентные Supabase/Yandex
  mutations.
- #1018 разрешил новые custom exercises в Live; #1019 перенёс замеры и графики
  в обзор Progress; #1020 добавил каталог готовых тренировок; #1023 упростил
  итог тренировки; #1026 поднял CTA готовой тренировки до уровня voice-блока.
- Расписание сначала показывает всю неделю сеткой `Пн/Вт`, `Ср/Чт`,
  `Пт/Сб`, `Вс`: все тренировки видны строками «время + имя», пары
  симметричны и растут без внутреннего скролла. Нажатие открывает прежнее
  подробное расписание выбранного дня. В нём назначение тренера, которое
  спортсмен сам начал и завершил, явно отмечено спокойным серо-синим тоном;
  старые записи без сохранённых авторов не перекрашиваются.
- Основной тренер архивирует клиента из списка только после явного «В архив»:
  свайп влево или доступная кнопка `⋯` лишь открывают действие. Есть «Вернуть»
  и восстановление архивной карточки; connected trainer управления не видит.
- Яндекс Метрика получает только внутренний `userId`, роль и
  `authenticated_open`; localhost, Preview и browser tests исключены из
  production-статистики.
