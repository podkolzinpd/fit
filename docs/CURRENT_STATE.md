...
Проверенный базовый `main`: `6f88140` (`feat(yandex): add client overview refresh (#576)`)

## Активное изменение

- YAFIT-369 добавляет в тренерский ИИ-анализ общую карту тела из Client
  Progress. Карта получает пол выбранного клиента, факты текущей внутренней
  сводки и завершённые подходы её фактического периода.
- Формулы прогресса и нагрузки, геометрия фигур, состояния и права доступа не
  дублируются: обе роли используют один компонент и один расчётный контракт.
- Генерация, публикация и содержание ИИ-сводки не меняются.
