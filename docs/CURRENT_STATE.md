...
После #1181 подготовлены offline release и локальный HTTP-стенд. По отдельному разрешению создан изолированный private frontend probe: бакет 1 ГБ, API Gateway и bucket-scoped reader без ключей. Cloud smoke подтвердил GET/HEAD, SPA fallback, JS recovery, asset 404, точные fixtures и no-store; анонимный storage GET запрещён. Gateway не выдаёт ETag, проверенные conditional GET возвращают 200, не 304. Immutable assets, реальный release и атомарная activation ещё не проверены. Жёсткий предел 600 ₽ снят, домен позже; Vercel/DNS/production не менялись. Факты и гейты: `docs/design/YANDEX_FRONTEND_CLOUD_PROBE.md`.
...
- Client Home показывает компактную карту «Нагрузка по телу»: фигура слева, зона/процент/«За последний месяц» с точными датами справа. Раскрываемые пояснение процента и выбор зоны сохраняют доступ к деталям без вложенных капсул. «Открыть в прогрессе» ведёт к той же месячной нагрузке; exact-workout срез не возвращён. Все три маршрута используют общие калиброванные SVG-контуры и исходные фотографии; настройки «Фигура / Список» мигрируют прежнюю схему без потери показателей и без подстановки пола.
