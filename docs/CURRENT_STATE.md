...
Проверенный базовый `main`: `72c4478` (`fix(auth): очищать устаревшую локальную сессию перед входом (#742)`)
...
- Карточка техники использует локальное зацикленное видео для четырёх упражнений
  пилота Vital Animations с точным совпадением движения. При ошибке, отсутствии
  видео или включённом уменьшении движения сохраняется кадровый fallback.
