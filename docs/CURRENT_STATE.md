...
Проверенный базовый `main`: `2863bd6` (`fix(yandex): preflight runtime database identity (#592)`)

## Активное изменение

- Client Progress и Trainer Progress переведены на один фактический контракт:
  лучший результат, три показателя, карта тела, сравнение периодов, связь с
  целью, ближайший план, один вывод и подробности в нижнем листе.
- Роль меняет только действия и безопасный текст: тренеру остаются plan/fact,
  сигналы, редактор и публикация; клиент не получает внутренние замечания.
- Настройка фигуры остаётся независимой у клиента и тренера. Генерация ИИ,
  расчёты прогресса, права доступа и публикация не меняются.
