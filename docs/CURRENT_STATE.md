...
- `YAFIT-357` возвращает тренеру возможность сохранить сразу завершённую
  тренировку на любую выбранную дату, включая будущую, без скрытого переноса
  на сегодня.
- `YAFIT-356` уже слит: клиентский экран проверки после клавиатуры сохраняет
  полную высоту на iPhone. Следом идёт `YAFIT-358` — запись факта пропущенного
  плана без Live.
...
1. Завершить `YAFIT-357` отдельным PR, проверить mobile WebKit и production.
2. Затем отдельно выполнить `YAFIT-358` — запись факта без Live.
