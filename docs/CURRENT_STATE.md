...
Обновлено: 2026-09-21. База изменений: `fcdffeac` (#1132). Frontend остаётся на Vercel, а production data plane — принятый Yandex Cloud stage stack.
...
- Stage delivery подтверждает `min_instances` активной revision и проверяет её `/health` без retry. Application failure откатывает кандидата; platform failure без `x-fit-request-id` делает deploy красным, но не возвращает прошедший smoke API к непрогретой ревизии. До warmup непрерывный gate прошёл 755 `/health` и остановился на запросе 756/1000 с platform `502`; отдельный запрос после пяти минут простоя также завершился до Fastify, а следующий сразу вернул `200`. Ручная диагностика различает `platform_invocation/handler=not_started` и application handler. После выпуска warmup окончательный гейт — 1 000 `/health` за 50 минут без retry и `502` на одной revision.
