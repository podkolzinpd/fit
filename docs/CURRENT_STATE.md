...
Обновлено: 2026-09-24. База изменений: `1b0ec9f5` (#1155). Frontend остаётся на Vercel, а production data plane — принятый Yandex Cloud stage stack.
...
- Карта последней тренировки на Client Home показывает компактное превью «Нагрузка по телу»: все рассчитанные зоны видны на фигуре без общего счётчика и рейтинга мышц, а «Открыть в прогрессе» сохраняет точную тренировку, дату и режим нагрузки (YAFIT-538).
