...
Проверенный базовый `main`: `91605e1` (`полный Vital Free50, #748`)
...
- Расширение содержит 120 уникальных карточек и 240 валидных локальных JPG;
  итоговый каталог — 662 упражнения без дублирования Vital-лестницы.
- Chromium 2/2, iPhone WebKit 1/1 и визуальные 390/430/1440 зелёные; ручной
  browser QA подтвердил поиск, обе фотографии и отсутствие переполнений.
- `npm run check`, SQL/RLS 886/886 и API actor/RLS 28/28 зелёные.
## Ближайший порядок
1. Создать PR расширения фотокаталога и дождаться всех обязательных проверок.
2. Дождаться зелёного CI, слить #749 и проверить production.
3. Установить свежую сборку из нового `main` в iOS Simulator.
## Отложено
