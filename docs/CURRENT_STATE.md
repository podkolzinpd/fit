...
Проверенный базовый `main`: `3855516` (`YAFIT-341: сделать завершение тренировки полезным (#526)`)

## Активное продуктовое изменение

- `YAFIT-356` устраняет повторное сжатие клиентского экрана проверки тренировки
  после цифровой клавиатуры на iPhone. Мобильная оболочка держит полную высоту
  корневого экрана, CTA остаётся доступным после ввода.
- Следом отдельными изменениями идут `YAFIT-357` (любая дата завершённой
  тренировки) и `YAFIT-358` (запись факта пропущенного плана без Live).
...
- AppLayout: 16 unit-тестов зелёные.
- Клиентский экран проверки после фокуса и закрытия клавиатуры проверен в
  WebKit на 390 и 430 px: оболочка равна высоте viewport, CTA достижим,
...
1. Завершить `YAFIT-356` отдельным PR, проверить mobile WebKit и production.
2. Затем отдельно выполнить `YAFIT-357` и `YAFIT-358`, по одному PR на задачу.
3. Не начинать `YAFIT-350–354` до завершения внешней задачи по ИИ-составлению
   программ и нового решения владельца продукта.
4. Инфраструктурный перенос Yandex продолжается независимо; production UI пока
   использует Supabase до отдельного cutover-решения.
5. `YAFIT-333` и `YAFIT-334` отложены; аналитика основного цикла остаётся в
   конце программы.
