...
Проверенный базовый `main`: `5a92d955` (`feat(progress): move the detailed analysis below management (#713)`)
...
- YAFIT-421, пункт 2: компактный период и единая карточка «Главное сейчас +
  цель». Локально зелёные `npm run check`, `db:test`, client Progress e2e,
  WebKit и visual baselines на 390/430 px; перед merge обязательны `app`,
  `database`, `e2e` и production smoke.
## Ближайший порядок
1. Завершить пункт 2 YAFIT-421: PR, CI, merge и production smoke.
2. Только после production перейти к пункту 3 YAFIT-421 — компактной карте тела.
