...
Проверенный базовый `main`: `45ca8c7` (`YAFIT-323: упростить замеры и показатели (#480)`)
## Активная работа
...
- Отдельная серия улучшений Progress идёт по согласованному плану. `#474`
  ...
  реальному периоду от первой завершённой тренировки. `#476` перевёл
  ИИ-контракт на русские предметные формулировки и закрыл старые технические
  ключи безопасным отображением, `#477` автоматизировал безопасную доставку
  функции через GitHub Actions без Docker, а `#478` перенёс измеримые сравнения
  в проверяемые серверные факты. `#479` унифицировал визуальную иерархию анализа
  для тренера и клиента. `#480` собрал замеры в один компактный блок:
  последнее значение и динамика видны сразу, а добавление, история и настройка
  показателей раскрываются отдельно без вложенных карточек. `YAFIT-324` завершает
  серию проверкой состояний, старых сводок, длинных названий, короткой истории
  и размеров 390/430/1440 px. Полный план сохранён в `docs/design/PROGRESS_SCREEN_PLAN_2026-08-20.md`.
