...
Проверенный базовый `main`: `9c8b585` (`feat(push): notify client instantly when their trainer schedules a workout (#688)`)
...
- Верх Progress закреплён в порядке `Период → Главное сейчас → Цель → Карта
  тела → Результаты периода`. Карта сохраняет режимы прогресса/нагрузки и виды
