...
Проверенный базовый `main`: `0955cd1` (`видео в карточке упражнения, #746`)
...
- Полный Vital Free50: 50 уникальных ID/роликов и 21 новая карточка проверены;
  медиа-аудит подтвердил H.264 540 × 540, 30 fps и отсутствие звука.
- Локальные Chromium 2/2 и iPhone WebKit 1/1 зелёные; ручной browser QA на
  390/430/1440 не выявил переполнений, видео работает в карточке и деталях.
- `npm run check`, SQL/RLS 886/886 и API actor/RLS 28/28 зелёные.
## Ближайший порядок
1. Дождаться зелёного CI Vital Free50, слить, проверить production и iOS.
2. Отдельным PR расширить фотокаталог без смысловых дублей.
3. После расширения каталога повторить браузерный и iOS QA.
## Отложено
