...
Проверенный базовый `main`: `28b7690` (`fix(yandex): verify exact stage candidate progress (#632)`)

## Активное изменение

- PR `#632` подтвердил exact candidate и fixture client. Stage run `33093878852`
  прошёл миграции и DB preflight, затем безопасно откатился после быстрого `503`.
- Логи и порядок smoke-запросов локализовали причину: чтение сохранённых
  `training-summaries` ошибочно зависело от `YANDEX_CLOUD_API_KEY`, хотя GET
  использует только PostgreSQL. Активный follow-up разделяет DB reader и AI
  generator; smoke получает отдельное имя для каждого progress endpoint.
...
- Полный `npm run check` зелёный: lint, typecheck, production build, 838 frontend,
  219 API и 57 infra/policy-тестов. 23 DB integration-теста корректно пропущены
  без локального PostgreSQL. Автоматический rollback сохранён.
