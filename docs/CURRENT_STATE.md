...
Обновлено: 2026-09-04. База задачи: `main` `ce504cf` (#781). Активная
задача: YAFIT-465 — удаление упражнения в Live и сохранение завершённого факта.
...
- YAFIT-465: обе роли получают удаление Live-упражнения с подтверждением;
  completed-save больше не сталкивает позиции оставшегося факта с планом,
  повторное редактирование не возвращает удалённые строки. Каталог не меняется.
- Локально прошли SQL/RLS 947, Yandex DB 29 и целевые WebKit-сценарии обеих
  ролей (Live conflict/retry и completed save/reopen через настоящую БД).
  План/границы: `docs/design/YAFIT_465_WORKOUT_EXERCISE_REMOVAL.md`;
  CI и production — отдельные обязательные проверки перед закрытием тикета.
