...
Обновлено: 2026-09-09. База: `main` `681b6c1`. Активно: Live — отдельный таймер, заметка клиента и стабильные строки; план `docs/design/LIVE_TIMER_NOTES_STABILITY.md`. Автономный выпуск разрешён пользователем.
...
- Trainer Progress сохраняет прежнюю композицию. У клиента личный факт сверху,
  полная карта ниже остальных разделов и свёрнута. С Home она открывает точную
...
- Live: check 1310/364/112; Supabase 998 assertions, Yandex PostgreSQL 30/30; Client 390/430 light/dark, WebKit offline/таймер/суперсет, переход и правка подтверждённого факта. Локальный reset запрещён auto-review; новая БД проверяется CI.
- YAFIT-480 закрыт: #827 + #828 / 178094d. YAFIT-481 закрыт: #829 / 95e61ce. YAFIT-482 закрыт: #831 / 1f7366e. YAFIT-483 закрыт: #832 / 3451d7e. YAFIT-484 закрыт: #833 / d10c965; CI 34318438943, Vercel 6344057251, public auth и свежий iOS проверены.
- YAFIT-485 локально: полный check 1305/364/112, 92 целевых component-теста
  и visual 390/430 px на Darwin/WebKit и Linux прошли; обновлены светлые и тёмные снимки.
...
## Ближайший порядок
1. YAFIT-480–485 в main. Текущая ветка Live сохраняет сворачивание завершённых и предстоящих упражнений.
2. Подходы редактируются напрямую; таймер не перерисовывает форму. Client note хранится отдельно от trainer cue в обоих backend.
3. Проверить CI; перед фронтендом выкатить additive Supabase migration, затем штатный merge, production и свежий iOS bundle.
4. Сессия consumer Tracker недоступна; полный утверждённый план хранится в docs/design и PR. Корпоративные credentials для YAFIT не использовать.
