...
Проверенный базовый `main`: `2ca6c5f` (`YAFIT-357: вернуть будущую дату завершённой тренировки (#528)`)

## Активное продуктовое изменение

- `YAFIT-358` переводит действие прошлого плана «Записать результат» на
  предзаполненную форму факта: та же тренировка завершается атомарно без Live и
  дубликата, а отмена оставляет план неизменным.
- `YAFIT-356` и `YAFIT-357` слиты: клиентский экран проверки после клавиатуры
  сохраняет полную высоту на iPhone, а завершённую тренировку снова можно
  сохранить на любую выбранную дату, включая будущую.
- `YAFIT-359` фиксирует отдельное продуктовое решение по каналу уведомлений о
  сообщениях формы обратной связи; сейчас они сохраняются в `app_feedback`.
...
- 596 SQL/RLS-тестов зелёные, включая атомарную запись результата, сохранение
  plan/fact, optimistic conflict, запрет постороннего доступа и отсутствие
  дубликата.
- Мобильный WebKit-сценарий на 390 px подтверждает: прошлый план открывает
  заполненную форму факта, сохраняется со статусом «Готово», не переходит в
  `/live` и не создаёт горизонтального переполнения.
...
1. Завершить `YAFIT-358` отдельным PR, проверить CI, merge и production.
