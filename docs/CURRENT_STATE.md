...
Проверенный `main`: `811a942` (`feat: parse running workout phrases locally (#417)`)
...
- После `#417` быстрый текстовый и голосовой ввод локально разбирает короткие
  беговые фразы, интервалы, восстановление и смешанные связки через `+` без
  изменения LLM и SpeechKit.
- После `#416` Client Progress и карточка клиента у тренера показывают
  confirmed-only беговой прогресс: число пробежек, километраж, длительность,
  средний темп и RPE; темп сравнивается только для сопоставимых пробежек одного
  формата.
- После `#415` тренер и спортсмен явно выбирают формат бега, интервальную схему
  или ручную настройку; СБУ и смешанные тренировки остаются доступны.
...
- Обычная непрерывная пробежка использует время `мин:сек` / `ч:мм:сс` и
  дистанцию в метрах или километрах; производный темп одинаково отображается
  в quick review, плане, live, истории и копировании.
...
- `#415`–`#417`: merges `4eff6d0`–`811a942` подтверждены; обязательные GitHub
  checks и Vercel прошли для форматов бега, прогресса и локального разбора.
