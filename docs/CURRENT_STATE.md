...
- `YAFIT-347` завершает коррекцию быстрого ввода перед сохранением.
- Фраза «Жим гантелей на наклонной скамье 3 по 10 70 кг» выбирает гантели, а не штангу;
  явно названное оборудование защищено и в LLM merge.
- На экране проверки можно изменить порядок; связанные интервалы, суперсеты и круги
  перемещаются целым блоком, а добавление и переход дальше в этом режиме скрыты.
- Главные кнопки, SpeechKit, LLM prompt/schema/provider и сохранение не менялись.
...
4. В продуктовом бэклоге `YAFIT-333` и `YAFIT-334` отложены; `YAFIT-335`, `YAFIT-337` и `YAFIT-347`
   завершены. Аналитика основного цикла
