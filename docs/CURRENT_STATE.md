...
Проверенный базовый `main`: `7c9fcbf` (`feat(progress): compare equivalent periods (#694)`)
...
- Верх Progress закреплён в порядке `Период → Главное сейчас → Цель → Карта
  тела → Сравнение периодов → Измерения → Результаты периода`. Карта сохраняет режимы прогресса/нагрузки и виды
...
- Этап 5 YAFIT-414: deterministic unit/component — 45 тестов; client Progress
  Chromium — 6 сценариев; визуальная приёмка затронутых Progress-экранов —
  20 сценариев на 390/430/1440 px, light/dark; lint и typecheck зелёные.
- Отдельно проверены цель-связанный вес, стандартные и пользовательский
  показатель «Плечи», 0/1/несколько точек, устаревшие данные, период с более
  свежим замером после него, LLM validation/fallback, порядок блоков, touch
  targets и отсутствие горизонтального overflow на 320/375/390/430 px.

## Ближайший порядок

1. Завершить release gate этапа 5: полный check, PR, зелёный CI, merge,
   production deployment и smoke.
2. После подтверждённой выкладки перейти к этапу 6 — визуальной регулярности.
