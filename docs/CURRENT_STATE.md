...
Обновлено: 2026-09-26. База изменений: `db6bc875` (#1186). Frontend остаётся на Vercel, а production data plane — принятый Yandex Cloud stage stack.
Yandex ID является единственным production-входом; app-session, main routing и native registration включены глобально.

## Активная цель
Восстановить обложки/анимации после задержки или отказа загрузки. WebKit воспроизвёл белые карточки велотренажёра и жима ногами на фактических public-путях `/exercises/vital/`; прежний smoke проверял другие, private-копии. Исправление ждёт реального image load, ограничивает зависший запрос 10 секундами, повторяет один раз и завершает loading нейтральной пиктограммой после отказа обоих кадров. Приватные URL обновляются до истечения, после online/возврата допускается ограниченное восстановление. Маршрутизация, каталог и данные тренировок не меняются.
