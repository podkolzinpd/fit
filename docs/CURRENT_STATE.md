...
- Platform-level `502` остаётся ошибкой Yandex runtime до входа в HTTP-процесс: `min_instances=1`, короткий idle timeout и закрытие socket после каждого ответа не устранили её полностью. Непрерывный gate прошёл 755 `/health` и остановился на запросе 756/1000 с `502` без `x-fit-request-id`; отдельный реальный инцидент также завершился за 28,975 мс без Fastify method/path. Ручная диагностика теперь различает `platform_invocation/handler=not_started` и application handler, показывает `fit-stage-api`, активные на момент проверки revision/release и известную безопасную операцию.
