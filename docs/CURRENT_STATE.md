...
Обновлено: 2026-09-03. Проверенный базовый `main`: `f5bdd44` (#772 выкачен); активная задача #774 — возвраты в тренировках и календарь истории у тренера.
...
- Picker скрывает четыре точных дубля системных карточек без удаления старых ref. Пользовательские упражнения не объединяются с системными даже при совпадающем имени или ref; история разрешает полный каталог.
...
- #772: merge + production + Deploy database успешны; создание своего упражнения клиентом исправлено без изменения ID/истории тренеров.
