...
Обновлено: 2026-09-09. База: `main` `1f7366e`. Активно: YAFIT-480–483, автономно утверждены все четыре этапа Home/Progress.
## Последняя проверенная продуктовая точка
- YAFIT-480: Home, Progress и завершение используют общий подтверждённый личный факт без ожидания ИИ: база/изменение/рекорд, две записи и даты. После save/edit/delete обновляются производные запросы. Старая сводка подписана отдельно. Первая запись больше не PR в обоих backend. YAFIT-481 добавляет компактную карту последней тренировки на Home; затем 482 и 483 без согласований.
...
- YAFIT-480 закрыт: #827 + #828 / 178094d, CI/Vercel/DB и повторный Yandex 34285336984 зелёные, свежий iOS запущен. YAFIT-481 закрыт: #829 / 95e61ce, CI 34293704252, Vercel 6339987108, public auth и свежий iOS проверены. YAFIT-482 закрыт: #831 / 1f7366e, CI 34297235553, Vercel 6340459799, public auth и свежий iOS проверены.
...
## Ближайший порядок
1. YAFIT-480–482 выпущены. YAFIT-483 реализован в #832; итоговый статус выпуска — в этом PR и Tracker.
2. Выпуск #832 включает обязательный CI, штатный production и свежий iOS из подтверждённого main.
3. YAFIT-483: check 1305/364/112 passed; 22 Darwin/WebKit и 21 Linux сценарий. Быстрые фильтры, источники и пересчёт после edit/delete проверены.
4. После выпуска — пользовательский пилот; новая разработка только по следующим корректировкам владельца.
