...
Обновлено: 2026-09-09. База: `main` `234655e`. Активно: плотная таблица подходов Live после #838. Постоянное разрешение на автономный выпуск зафиксировано в `AGENTS.md`.
...
- YAFIT-486 выпущен: #836 / b86b097, повторный CI 34341056568 и Vercel 6348157460 успешны; public auth и свежий iOS проверены.
- YAFIT-487: «Посмотреть анализ» прокручивает только внутренний `.content`, не меняет hash и возвращает root scroll в 0. ИИ, период и таббар не пересоздаются.
- Полный check: frontend 1314, API 364 (+30 skipped), infra 112; lint/typecheck/build и проверки DB types/iOS permissions пройдены. iPhone/WebKit и Android/Chromium проверяют видимость ИИ-блока, нулевой root scroll и положение таббара у нижнего края; visual 390/430 не изменился.
## Ближайший порядок
1. Запустить полный check, открыть PR YAFIT-487 и дождаться зелёного CI.
2. Слить, проверить production и свежий iOS bundle; закрыть YAFIT-487.
