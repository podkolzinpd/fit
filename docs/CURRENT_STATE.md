...
- Trainer Progress добавляет после следующего шага отдельный свёрнутый блок:
  максимум три проверяемых сигнала, у каждого отдельно видны факт и вопрос для
  разговора. Клиент не получает блок; публикация его версии не меняется.
...
- Этап 8 YAFIT-414: полный check — 977 frontend, 230 API, 66 infra; DB/RLS
  886, actor API/RLS 23; Chromium 6, WebKit 8, visual 390/430/1440 зелёные.

## Ближайший порядок

1. Завершить release gate этапа 8: полный check, PR, CI, merge и production smoke.
2. После подтверждённой выкладки перейти к этапу 9 — подробному анализу и аудиту.
