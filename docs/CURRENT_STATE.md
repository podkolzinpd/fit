...
Проверенный базовый `main`: `0259b65` (`YAFIT-333: отложить перестройку списка клиентов (#513)`)

## Последнее продуктовое изменение

- Новый спортсмен начинает с микрофона или текста, а не с длинной анкеты;
  минимальная самостоятельная карточка создаётся только при первом действии и
  не содержит выдуманных антропометрических данных.
- Новый тренер добавляет первого спортсмена одним именем, сразу составляет ему
  первый план и после сохранения получает приглашение.
- Ссылка приглашения сохраняется через авторизацию и требует понятного явного
  подтверждения «Подключиться и открыть план».
- Главные страницы обеих ролей по-прежнему сохраняют микрофон и ввод текстом;
  LLM и SpeechKit не менялись.
...
4. В продуктовом бэклоге `YAFIT-333` и `YAFIT-334` отложены; `YAFIT-335`
   завершена в текущем PR. Аналитика основного цикла остаётся в конце программы.
