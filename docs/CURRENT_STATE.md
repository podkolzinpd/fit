...
Проверенный базовый `main`: `de9096f` (`YAFIT-414: add trainer-only verified Progress signals (#710)`)
...
- `app_feedback`: 82 целевых route-теста и 24 локальных PostgreSQL actor/RLS
  integration-теста зелёные. Итоговый `npm run check`: 977 frontend, 238 API и
