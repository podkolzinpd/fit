...
Обновлено: 2026-09-09. База: `main` `0574750`. Активно: YAFIT-486, простые разделы Client Progress. Постоянное разрешение на автономный выпуск зафиксировано в `AGENTS.md`.
...
- YAFIT-484 выровнял переключатель сторон компактной карты на Home: равные
  сегменты, зона нажатия 44 px и стабильная сетка на ширинах 360–430 px.
...
- Live сохраняет каждое изменение подхода на устройстве, автоматически досылает pending-черновики и ограничивает зависшие запросы. Finish сначала отправляет
  открытую строку; ошибка закреплена у повтора, а кнопка крупная на 360–430 px.
...
- YAFIT-480–485 выпущены; #834 / 681b6c1, CI 34330951767 и Vercel 6346330604 успешны, public auth и свежий iOS проверены.
- YAFIT-486: план полностью зафиксирован в docs/design/CLIENT_PROGRESS_SIMPLE_SECTIONS.md и Tracker. Client Home, тренерская композиция, AI/API-контракты сохранены.
- Полный check: frontend 1308, API 364 (+30 skipped), infra 112; lint/typecheck/build и проверка DB types/iOS permissions пройдены.
- 24 browser-сценария прогресса прошли с актуальными именами действий: Android/Chromium и iPhone/WebKit 320/375/390/430, источники/фильтры, AI error, замеры и trainer regression.
- 24 visual-сценария 390/430 light/dark пройдены на macOS и Linux; финальные снимки перечитаны. Linux WebKit под amd64-эмуляцией потребовал повторов после Target crashed; финальные целевые 8 сценариев прошли.
## Ближайший порядок
1. PR #836: CI 34339014912 прошёл; после объединения с main дождаться повторного CI, штатно слить и проверить production.
2. Из подтверждённого main обновить и проверить iOS bundle; закрыть YAFIT-486.
