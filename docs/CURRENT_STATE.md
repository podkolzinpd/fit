...
Проверенный `main`: `7fa7a36` (`YAFIT-305: переключатель км/м читается на iPhone (#405)`)

## Активная работа

- Активной задачи нет. Следующая согласованная задача бегового блока:
  `YAFIT-301` — интервалы, восстановление и СБУ; пользователь разрешил начать.
...
- `#403` / `YAFIT-304`: merge `d4278b7` подтверждён; GitHub CI app/database/e2e
  и Vercel прошли. Локально `npm run check` — 432 теста приложения, 10 тестов
  API, lint/typecheck/DB types/iOS permissions/build; WebKit quick review
  тренера и клиентский выбор/запуск бега на 390 px прошли, визуальные профили
  Client 390/430 и Trainer 1440 зелёные. БД, LLM и SpeechKit не менялись.
...
1. `YAFIT-301` — интервалы, восстановление и СБУ.
2. `YAFIT-302` — беговой прогресс: километраж, длительность, темп и RPE.
3. `YAFIT-303` — текстовые и голосовые сценарии бегового ввода.
4. `YAFIT-290` — actionable-список «Кому нужно внимание» для тренера.
