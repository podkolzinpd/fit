...
Обновлено: 2026-09-09. База: `main` `de3aa1d`. Активно: YAFIT-480–483, автономно утверждены все четыре этапа Home/Progress.
## Последняя проверенная продуктовая точка
- YAFIT-480: Home, Progress и завершение используют общий подтверждённый личный факт без ожидания ИИ: база/изменение/рекорд, две записи и даты. После save/edit/delete обновляются производные запросы. Старая сводка подписана отдельно. Первая запись больше не PR в обоих backend. Следом YAFIT-481: компактная карта на Home; затем 482 и 483 без согласований.
...
- Remote audit `34229105217` выбрал cohort: 1 клиент, 5 строк в 28 таблицах.
  Dry-run `34244770314` после merge пропуска конфликтов безопасно откатил 10
  stale stage cohorts и подтвердил, что прежний лимит был недостаточен;
  configured cohort остаётся пустым.
## Ближайший порядок
1. Влить расширенный bounded scan конфликтующих stage cohorts и получить зелёный remote dry-run.
2. Зафиксировать выбранный cohort и только затем отдельно подтвердить stage apply.
3. Связать Yandex ID выбранного tenant и проверить основной UI через sticky routing.
4. После одного успешного tenant перевести оставшихся пользователей партиями;
