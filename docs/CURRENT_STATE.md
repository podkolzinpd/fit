...
Обновлено: 2026-09-07. База: `main` `520dc634`. Активно: безопасный автоматический выбор непустого tenant для remote audit/dry-run.
...
- Каталог сохраняет 663 прежних ref и добавляет 317 проверенных Gym Pro
  ...
  содержит 312 карточек. Основной выбор строится на анимированном каталоге;
  старые идентификаторы, варианты, синонимы и custom-карточки не теряются и
  доступны в поиске/истории. Все строки статичны до тапа, проигрывается только
  последняя нажатая; `+ / ✓` по-прежнему независимо меняет выбор.
