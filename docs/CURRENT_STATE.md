...
Проверенный базовый `main`: `a098043` (`ci: add manual Vercel PR previews (#696)`)
...
- Верх Progress закреплён в порядке `Период → Главное сейчас → Цель → Карта
  тела → Сравнение → Измерения → Регулярность → Результаты`. Карта сохраняет режимы прогресса/нагрузки и виды
...
- Этап 6 YAFIT-414: regularity unit/component — 32 теста; client Progress
  Chromium — 6, WebKit — 9 сценариев; Linux/Darwin visual на 390/430/1440 px,
  light/dark; SQL/RLS — 885 тестов; lint и typecheck зелёные.
- Отдельно проверены недели с понедельника, активность/пропуски без будущих
  недель, интервалы, пауза, серия, равный прошлый период, пять допустимых
  pattern, строгий LLM fallback, порядок и overflow на 320/375/390/430 px.

## Ближайший порядок

1. Завершить release gate этапа 6: полный check, PR, зелёный CI, merge,
   production deployment и smoke.
2. После подтверждённой выкладки перейти к этапу 7 — следующему шагу.
