...
- В Live остаётся один общий статус тренировки. Компактное `Ввести` является
  единственным заполненным коралловым действием текущего подхода, добавление
  подхода нейтрально, а таймер получает акцент только во время отдыха.
  Данные, подтверждение подходов и RPC не меняются.
