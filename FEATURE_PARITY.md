...
| Chat | Личный текстовый диалог спортсмена с каждым тренером, история, непрочитанные, черновики, надёжная отправка и переход из push | Implemented: общий Supabase/Yandex контракт, participant-only RLS, idempotent retry, realtime/polling/foreground refresh, multi-device Web Push deep-link и продолжение переписки после отключения без возврата доступа к данным; Chromium/WebKit и visual 390/430/1440 px |
