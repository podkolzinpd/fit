...
alter table public.chat_messages add constraint chat_messages_content_valid check (
  body=btrim(body) and char_length(body)<=4000 and (
...
language sql stable security definer set search_path='' as $$
  ...
  select conversation.id,available.client_id,available.trainer_id,
    ...
    (client.trainer_id=available.trainer_id and client.trainer_id<>client.auth_user_id)
      or exists(select 1 from public.client_trainers membership where membership.client_id=available.client_id and membership.trainer_id=available.trainer_id),
...
declare actor_id uuid:=auth.uid(); stored_path text;
begin
  select message.image_path into stored_path from public.chat_messages message
    join public.chat_conversations conversation on conversation.id=message.conversation_id
    where message.id=p_message_id and message.conversation_id=p_conversation_id and message.sender_id=actor_id
  ...
  update public.chat_messages set body='',deleted_at=coalesce(deleted_at,now()) where id=p_message_id;
  return stored_path;
end; $$;

revoke all on function public.delete_chat_message(uuid,uuid) from public;
grant execute on function public.delete_chat_message(uuid,uuid) to fit_api;

-- Down Migration

revoke execute on function public.delete_chat_message(uuid,uuid) from fit_api;
drop function public.delete_chat_message(uuid,uuid);
...
language sql stable security definer set search_path='' as $$
  ...
  select conversation.id,available.client_id,available.trainer_id,
    ...
    case when conversation.id is null then 0 else (select count(*) from public.chat_messages unread where unread.conversation_id=conversation.id and unread.sender_id<>actor.id
      and unread.created_at>coalesce(case when actor.id=conversation.client_user_id then conversation.client_last_read_at else conversation.trainer_last_read_at end,'-infinity'::timestamptz)) end
  from available join public.clients client on client.id=available.client_id cross join actor
  left join public.chat_conversations conversation on conversation.client_id=available.client_id and conversation.trainer_id=available.trainer_id
  join public.profiles partner on partner.id=case when actor.id=client.auth_user_id then available.trainer_id else client.auth_user_id end
...
begin
  ...
  return query select message.id,message.conversation_id,message.sender_id,message.body,message.created_at,
    message.image_path,message.image_mime_type,message.image_width,message.image_height,message.image_size_bytes
    from public.chat_messages message where message.conversation_id=p_conversation_id
      and (p_before_created_at is null or (message.created_at,message.id)<(p_before_created_at,p_before_id))
    order by message.created_at desc,message.id desc limit least(greatest(p_limit,1),100);
end; $$;

update public.chat_messages set body='Сообщение удалено' where deleted_at is not null and body='';
alter table public.chat_messages drop constraint chat_messages_content_valid;
