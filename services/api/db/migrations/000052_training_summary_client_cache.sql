...
begin
  ...
  if client_row.id is null or actor_id is distinct from client_row.auth_user_id then
  ...
  insert into public.client_published_training_summaries (
    ...
    source_row.display_metrics, source_row.input_fingerprint,
    source_row.generated_at, now(), null
...
revoke all on function public.publish_cached_training_summary_for_client(uuid, date, date, text, text) from public;
grant execute on function public.publish_cached_training_summary_for_client(uuid, date, date, text, text) to fit_api;

-- Down Migration

revoke execute on function public.publish_cached_training_summary_for_client(uuid, date, date, text, text) from fit_api;
drop function public.publish_cached_training_summary_for_client(uuid, date, date, text, text);
