...
revoke all on function public.list_chat_messages_v3(uuid,timestamptz,uuid,integer),
  ...
  public.get_chat_message_window(uuid,uuid,integer) from public;
grant execute on function public.list_chat_messages_v3(uuid,timestamptz,uuid,integer),
  public.send_chat_message_v3(uuid,uuid,text,text,text,integer,integer,integer,uuid),
  public.edit_chat_message(uuid,uuid,text), public.get_chat_unread_state(uuid),
  public.mark_chat_read_v2(uuid,uuid), public.search_chat_messages(uuid,text,integer),
  public.get_chat_message_window(uuid,uuid,integer) to fit_api;

-- Down Migration

revoke execute on function public.list_chat_messages_v3(uuid,timestamptz,uuid,integer),
  public.send_chat_message_v3(uuid,uuid,text,text,text,integer,integer,integer,uuid),
  public.edit_chat_message(uuid,uuid,text), public.get_chat_unread_state(uuid),
  public.mark_chat_read_v2(uuid,uuid), public.search_chat_messages(uuid,text,integer),
  public.get_chat_message_window(uuid,uuid,integer) from fit_api;
drop function public.get_chat_message_window(uuid,uuid,integer);
drop function public.search_chat_messages(uuid,text,integer);
drop function public.mark_chat_read_v2(uuid,uuid);
drop function public.get_chat_unread_state(uuid);
drop function public.edit_chat_message(uuid,uuid,text);
drop function public.send_chat_message_v3(uuid,uuid,text,text,text,integer,integer,integer,uuid);
drop function public.list_chat_messages_v3(uuid,timestamptz,uuid,integer);
drop index public.chat_messages_conversation_body_search_idx;
drop function public.list_chat_threads();
drop function public.mark_chat_read(uuid);
alter table public.chat_messages drop column reply_to_message_id, drop column edited_at;
alter table public.chat_conversations drop column trainer_last_read_message_id, drop column client_last_read_message_id;
...
returns table (
  ...
  active_connection boolean,last_message_body text,last_message_at timestamptz,last_message_sender_id uuid,unread_count bigint
)
language sql stable security definer set search_path='' as $$
  ...
  select conversation.id,available.client_id,available.trainer_id,
    ...
    case when conversation.id is null then 0 else (select count(*) from public.chat_messages unread where unread.conversation_id=conversation.id
      and unread.sender_id<>actor.id and unread.deleted_at is null and unread.created_at>coalesce(case when actor.id=conversation.client_user_id
        then conversation.client_last_read_at else conversation.trainer_last_read_at end,'-infinity'::timestamptz)) end
  from available join public.clients client on client.id=available.client_id cross join actor
  left join public.chat_conversations conversation on conversation.client_id=available.client_id and conversation.trainer_id=available.trainer_id
  join public.profiles partner on partner.id=case when actor.id=client.auth_user_id then available.trainer_id else client.auth_user_id end
...
create function public.mark_chat_read(p_conversation_id uuid)
...
grant execute on function public.mark_chat_read(uuid) to fit_api;
