...
-- Failed attempts from the pre-000059 contract may still carry a stale
-- reservation count. They must not consume today's successful-generation
-- budget, and a retry of a failed guard starts a fresh reservation.
...
begin
  ...
  insert into app_private.training_summary_generation_guards (
    ...
    model_calls_today = case
      when app_private.training_summary_generation_guards.status = 'failed'
        or app_private.training_summary_generation_guards.model_calls_day <> utc_day
        then 1
      else app_private.training_summary_generation_guards.model_calls_today + 1 end,
