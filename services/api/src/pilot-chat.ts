...
export type ChatMessage = { id: string; conversationId: string; senderId: string; body: string; createdAt: string }
...
type MessageRow = QueryResultRow & { id: string; conversation_id: string; sender_id: string; body: string; created_at: string }
...
function message(row: MessageRow): ChatMessage { return { id: row.id, conversationId: row.conversation_id, senderId: row.sender_id, body: row.body, createdAt: row.created_at } }
...
export interface PilotChat {
  ...
  send(session: YandexActorSessionInput, conversationId: string, messageId: string, body: string): Promise<ChatMessage>
...
export class DatabasePilotChat implements PilotChat {
  ...
  listMessages(session: YandexActorSessionInput, conversationId: string, cursor: ChatCursor | null, limit: number) {
    return this.run(session, async (client) => {
      const rows = await client.query<MessageRow>('select * from public.list_chat_messages($1,$2,$3,$4)',[conversationId,cursor?.createdAt ?? null,cursor?.id ?? null,limit])
  ...
  send(session: YandexActorSessionInput, conversationId: string, messageId: string, body: string) {
    return this.run(session, async (client) => {
      const rows = await client.query<MessageRow>('select * from public.send_chat_message($1,$2,$3)',[conversationId,messageId,body])
