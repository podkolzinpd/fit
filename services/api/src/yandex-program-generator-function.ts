import { programPlanFeedback, programPlanSchema, readProgramPlan, replaceProgramDay } from './assistant-orchestrator/program/plan.js'
...
Если limitations=present/unknown, используй limitationsText и limitationAdjustments при выборе упражнений и назначений. Соблюдай явно заданные исключения и допустимые варианты; не возвращай исключённые упражнения и не заменяй их вариантом с той же запрещённой нагрузкой. Объясни конкретные изменения в rationale и заметках упражнений. Не ставь диагноз и не придумывай медицинские рекомендации. Если допустимая нагрузка пока неизвестна, обозначь неопределённость и что тренеру нужно проверить перед выполнением; не называй программу медицински безопасной. Само наличие ограничения не является причиной отказа: составь черновик для проверки тренером из доступных совместимых вариантов.
...
Выбирай состав и распределение движений по цели, опыту, истории и предпочтениям клиента; объясни выбор и акценты дней. Универсального обязательного набора движений нет. Не добавляй упражнение только ради заполнения шаблона. На день 3–8 упражнений. По одному упражнению не повышай сразу несколько осей (подходы/повторы/усилие). Рост за неделю: не более 1 подхода, 2 повторов, 10 секунд или 0.5 RPE; общий объём подходов максимум +20%. Можно сохранять или снижать.
Исторические meanWeeklyCatalogSets и meanWeeklyWorkouts — наблюдения, не потолок новой программы. Неполные записи не доказывают низкую переносимость нагрузки. Выбирай объём по цели, опыту, перерыву, ограничениям и частоте; при отличии от истории объясни причину в rationale. Не увеличивай объём автоматически из-за роста числа дней. Держись границ load.maxSetsPerExercise и load.rpe. Для повторов и секунд максимум увеличения относительно первой недели указан в load.increments; для секунд умножить на 5. Это потолок, не обязательная прогрессия. Схема задаёт допустимые последовательности: для каждого упражнения самостоятельно выбери числа, вариант развития, удержания или снижения. Подходы и RPE в пределах блока оставляй постоянными для данного упражнения; между упражнениями они могут различаться. В стартовом режиме первые две недели знакомство и закрепление с тренером, без роста повторов, третья/четвёртая — развитие или удержание/снижение по условиям.
На день максимум 18 подходов для beginner/returning, 24 для experienced. Не ставь больше одного unsupportedTrunk упражнения с RPE>=7 за день. RPE кратно 0.5, отдых 60–180 секунд. Время оценивается как 10 минут разминки + сумма (2 минуты на упражнение + подходы*(повторы*3 либо секунды удержания + отдых)/60). Уложись в brief.durationMin. Если тесно — меньше упражнений/подходов, сохраняй недельный баланс.`
    ...
    const generate = (repair?: { previousPlan: unknown; issues: string[]; diagnostics: ReturnType<typeof programPlanFeedback>; targetWeekday?: number }) => programModelJson({
      instruction: instruction + (repair?.targetWeekday !== undefined ? `\nИсправь только день ${repair.targetWeekday}. Верни sessions ровно с этим днём и 3–8 упражнений для него; все строки упражнений имеют weekday=${repair.targetWeekday}. Остальные дни сохранены отдельно. Это часть многодневной программы, не отдельная тренировка всего тела. Не меняй ограничения клиента.` : '') + (repair ? '\nПредыдущий ответ отклонён валидатором. Исправь все ошибки из repair.issues; repair.diagnostics содержит измеренные числа упражнений по дням и требования. invalid_session_schema означает неверный день, пустой/длинный заголовок или число упражнений вне 3–8. Проверь каждый день по diagnostics; добавь или убери упражнения, если нужно, затем сохранив условия клиента и ограничения нагрузки. Верни полный исправленный JSON по той же схеме; не объясняй ошибку вместо программы.' : ''),
      data: { brief, context: body.context, load, catalog, ...(repair ? { repair } : {}) },
      schema: programPlanSchema(catalog, repair?.targetWeekday !== undefined ? { ...brief, frequency: 1, weekdays: [repair.targetWeekday] } : brief, load), maxTokens: 6500, timeoutMs: Math.max(1, deadline - Date.now()), functionName: 'fit-generate-program', operationId,
    ...
    try { template = validate(raw) }
      ...
      const diagnostics = programPlanFeedback(raw, brief)
      const invalidDays = error.codes.includes('invalid_session_schema') && 'sessions' in diagnostics
        ? diagnostics.sessions.filter((day) => typeof day.weekday === 'number' && brief.weekdays!.includes(day.weekday)
          && (day.exerciseCount < 3 || day.exerciseCount > 8)).map((day) => Number(day.weekday)) : []
      if (invalidDays.length && new Set(invalidDays).size === invalidDays.length) {
        const repairs = await Promise.all(invalidDays.map(async (targetWeekday) => ({ targetWeekday,
          plan: await generate({ previousPlan: raw, issues: error.codes, diagnostics, targetWeekday }),
        })))
        const repaired = repairs.reduce((plan, repair) => replaceProgramDay(plan, repair.plan, repair.targetWeekday), raw)
        template = validate(repaired)
      } else template = validate(await generate({ previousPlan: raw, issues: error.codes, diagnostics }))
    }
