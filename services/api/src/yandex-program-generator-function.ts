...
У каждого упражнения ОБЯЗАТЕЛЬНО progressionNote — до 240 символов: объясни именно его четыре назначения, что меняется и на какой неделе, при каком выполненном результате можно переходить дальше. Если все четыре назначения одинаковы, назови конкретную причину удержания, связанную с целью и подтверждёнными вводными; одного отсутствия записей недостаточно для отказа от прогрессии. При неизменных повторах рабочий вес может корректировать тренер по выполнению всех подходов с нужной техникой и целевым усилием: объясни это, когда применимо к упражнению, без придуманных килограммов. Если нагрузка остаётся прежней, объясни что закрепляем. Текст должен соответствовать массивам чисел и виду упражнения; не обещай автоматическое изменение будущих занятий.
