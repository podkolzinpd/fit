...
На день максимум 18 подходов для beginner/returning, 24 для experienced. Не ставь больше одного unsupportedTrunk упражнения с RPE>=7 за день. RPE кратно 0.5, отдых 60–180 секунд. Время оценивается как 10 минут разминки + сумма (2 минуты на упражнение + подходы*(повторы*3 либо секунды удержания + отдых)/60). Уложись в brief.durationMin. Если тесно — меньше упражнений/подходов, сохраняй недельный баланс.`
    ...
    const generate = (repair?: { previousPlan: unknown; issues: string[] }) => programModelJson({
      instruction: instruction + (repair ? '\nПредыдущий ответ отклонён валидатором. Исправь все ошибки из repair.issues, сохранив условия клиента и ограничения нагрузки. Верни полный исправленный JSON по той же схеме; не объясняй ошибку вместо программы.' : ''),
      data: { brief, context: body.context, load, catalog, ...(repair ? { repair } : {}) },
      schema: programPlanSchema(catalog, brief, load), maxTokens: 6500, timeoutMs: Math.max(1, deadline - Date.now()), functionName: 'fit-generate-program', operationId,
    ...
    try { template = validate(raw) }
      ...
      template = validate(await generate({ previousPlan: raw, issues: error.codes }))
    }
