...
export async function handler(event: Event): Promise<Result> {
  ...
  const response = await assistantOrchestrator(new Request('https://yandex-function.internal/assistant-orchestrator', { method: 'POST', headers: { authorization: authorization ?? '', 'content-type': 'application/json' }, body: raw }))
