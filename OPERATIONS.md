...
API задаёт Fastify `keepAliveTimeout=5000`: это намеренно короче периода
простоя, после которого provisioned Serverless Container может быть
приостановлен. Во время приостановки Yandex допускает потерю сетевых соединений;
поэтому нельзя возвращать Fastify default 72 секунды без повторной длительной
проверки — runtime сможет повторно использовать уже оборванный upstream socket
и вернуть `502 Error during function invocation` до входа запроса в приложение.
