...
Закрытый пилот светлой палитры из того же макета управляется отдельной парой
build-time переменных Vercel:

```text
VITE_LIGHT_THEME_PILOT_ENABLED=true
VITE_LIGHT_THEME_PILOT_USER_IDS=<auth-user-uuid-1>,<auth-user-uuid-2>
```

Механизм и правила те же, что у тёмной палитры: default-off, флаг должен быть
ровно `true`, список открыт тренерским и клиентским аккаунтам, изменение списка
требует нового deployment. Отличий два. Во-первых, палитра применяется, когда
участник пилота держит в профиле **светлую** тему (переключатель «Тёмная тема»
выключен) — а поскольку светлая тема стоит по умолчанию, отдельного действия от
участника обычно не требуется. Во-вторых, allowlist сознательно сделан отдельным
от тёмного: аудитории раскатываются независимо, аккаунт может быть в одном
пилоте и не быть в другом, и попадание в светлый список не открывает тёмную
пилотную палитру. Поле поиска в списке клиентов к этой паре переменных не
привязано и остаётся за `VITE_DARK_THEME_PILOT_*`.
