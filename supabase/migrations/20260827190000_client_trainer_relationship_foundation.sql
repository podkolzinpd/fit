-- Безопасный фундамент связи клиента с тренером.
--
-- Миграция намеренно добавочная: существующие clients.trainer_id,
-- client_trainers и invitation-функции сохраняются без изменения. Новый слой
-- станет источником истины после отдельной атомарной миграции привязки.
...
-- Переносим только уже связанные с аккаунтом карточки, у которых корневой
-- trainer_id не совпадает с самим клиентом. Самостоятельные клиенты остаются
-- без активного тренера. Исторические memberships пока не интерпретируем:
-- старый продукт разрешал несколько тренеров, а новый контракт — ровно одного.
...
revoke all on public.client_trainer_relationships,
  public.client_merge_operations from anon, authenticated;
grant select on public.client_trainer_relationships to authenticated;
...
-- client_merge_operations — внутренний аудит. Пользовательские роли не
-- получают к нему прямого доступа; task 2 будет писать туда из атомарной
