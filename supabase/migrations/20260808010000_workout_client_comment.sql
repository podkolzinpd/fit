-- Комментарий клиента хранится отдельно от инструкций, приватной заметки
-- тренера и отзыва тренера. Клиент может менять только свой комментарий к
-- собственной карточке; тренер получает его в уже доступной ему тренировке.
alter table public.workouts add column if not exists client_comment text;
...
begin
  ...
  update public.workouts workout
     set client_comment = nullif(btrim(p_comment), ''),
...
revoke all on function public.set_client_workout_comment(uuid, text, bigint) from public, anon;
grant execute on function public.set_client_workout_comment(uuid, text, bigint) to authenticated;

-- Расширяем единственный агрегированный список: карточки и тренерская история
-- должны получать тот же комментарий, что и детальный запрос.
...
returns table (
  ...
  status text, notes text, trainer_review text, client_comment text, version bigint, stage_id uuid, stage_title text,
...
begin
  ...
  with paged_workouts as materialized (
    select
      ...
      workout.status, workout.notes, workout.trainer_review, workout.client_comment, workout.version,
      workout.stage_id, stage.title as stage_title, workout.created_at, count(*) over() as total_count
    from public.workouts workout
  ...
  select
    workout.id, workout.client_id, workout.client_name, workout.workout_date, workout.start_time, workout.end_time,
    workout.started_at, workout.completed_at, workout.status, workout.notes, workout.trainer_review,
    workout.client_comment, workout.version, workout.stage_id, workout.stage_title, workout.total_count,
    coalesce((
