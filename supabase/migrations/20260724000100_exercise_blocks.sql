-- Блоки упражнений: сет / суперсет / трисет / круговая.
-- Упражнения с одним block_id образуют один блок; block_type задаёт вид.
-- Одиночные упражнения — свой уникальный block_id и block_type='single'.
-- Обратная совместимость: колонки с DEFAULT, старые вызовы без block валидны;
-- существующие упражнения бэкфиллятся как одиночные блоки.

alter table public.workout_exercises
  add column if not exists block_id uuid,
  add column if not exists block_type text not null default 'single';

-- Бэкфилл: каждому существующему упражнению — свой блок.
update public.workout_exercises set block_id = gen_random_uuid() where block_id is null;
...
alter table public.workout_exercises
  add constraint workout_exercises_block_type_allowed
  check (block_type in ('single', 'superset', 'triset', 'circuit'));

-- save_workout: читает block_id / block_type из JSON упражнения.
...
-- append_live_exercise: новое упражнение — свой одиночный блок.
...
begin
  ...
  insert into public.workout_exercises (
    ...
    custom_id, name_value, group_value, kind_value, gen_random_uuid(), 'single'
  ) returning id into exercise_id;
...
-- list_workouts: возвращает block_id / block_type в JSON упражнения.
