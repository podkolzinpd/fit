-- A failed paid attempt keeps the client-wide daily budget, but it must not
-- occupy the period slot after the cooldown expires. Successful and active
-- attempts still prevent duplicate generation for the same period.
