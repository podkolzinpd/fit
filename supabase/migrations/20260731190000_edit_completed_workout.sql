-- Завершённую тренировку можно исправить без возврата в live: сохраняем новый
-- факт, остаёмся в статусе done и не меняем исходное время завершения.
...
begin
  ...
  begin
    if workout_id_value is not null then
      -- legacy_save_workout безопасно пересобирает упражнения только у planned.
      -- В пределах одной транзакции временно переводим done → planned и сразу
      -- возвращаем done после пересборки факта.
