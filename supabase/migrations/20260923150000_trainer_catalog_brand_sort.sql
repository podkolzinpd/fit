-- Бренд-тренеры поднимаются в каталоге, но не в ущерб реальной доступности:
-- вторичный ключ сортировки после "берёт новых клиентов", а не вместо него -
-- иначе бренд-тренер, который клиентов не берёт, обгонял бы обычного
-- тренера, который берёт, и вёл бы клиента в тупиковую карточку. Сигнатура
-- функции не меняется - drop не нужен, права уже выданы прошлой миграцией.
