-- Older auth accounts can have a trainer profile without the one-to-one
-- trainer root. Enabling this explicitly reviewed pilot is also a bounded
-- repair opportunity; non-trainer and missing profiles remain rejected.
...
comment on function public.manage_trainer_schedule_v2_pilot(uuid, text) is
  'Service-role-only single-account Schedule V2 rollout control with bounded repair of a missing trainer root.';
