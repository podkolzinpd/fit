-- Отзыв тренера добавляется только после завершения тренировки. Он хранится
-- отдельно от общей заметки, чтобы клиент мог прочитать его, но не изменить.
alter table public.workouts add column if not exists trainer_review text;
...
declare
  ...
  next_version bigint;
begin
  -- Авторизация сохраняет обычные правила автора тренировки и не допускает
  -- клиентский путь (p_client_can_execute = false).
  root_trainer := public.authorize_workout_mutation(p_workout_id, false);

  update public.workouts workout
     set trainer_review = nullif(btrim(p_review), ''),
...
revoke all on function public.set_workout_review(uuid, text, bigint) from public, anon;
grant execute on function public.set_workout_review(uuid, text, bigint) to authenticated;

-- Изменение OUT-сигнатуры требует пересоздания функции. Сохраняем её
-- единственный агрегированный запрос и действующие ограничения доступа.
...
returns table (
  ...
  status text, notes text, trainer_review text, version bigint, stage_id uuid, stage_title text,
