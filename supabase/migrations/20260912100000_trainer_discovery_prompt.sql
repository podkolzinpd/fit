...
revoke all on table public.trainer_discovery_prompt_preferences from public, anon, authenticated;
...
create or replace function public.get_trainer_discovery_prompt()
returns jsonb
language plpgsql
stable
security definer
...
begin
  ...
  from public.trainer_discovery_prompt_preferences
  where user_id = actor_id;

  return jsonb_build_object(
    'state', case
      when preference.dismissed_at is not null then 'dismissed'
      when preference.remind_at > now() then 'snoozed'
      else 'visible'
    end,
...
create or replace function public.set_trainer_discovery_prompt(p_action text)
returns jsonb
language plpgsql
security definer
set search_path = ''
  ...
  if normalized_action = 'dismiss' then
    ...
    on conflict (user_id) do update set
      ...
      dismissed_at = coalesce(
        public.trainer_discovery_prompt_preferences.dismissed_at,
    ...
    on conflict (user_id) do update set
      remind_at = case
        when public.trainer_discovery_prompt_preferences.dismissed_at is null
          then excluded.remind_at
        else public.trainer_discovery_prompt_preferences.remind_at
      end;
  end if;
...
revoke all on function public.get_trainer_discovery_prompt() from public, anon;
revoke all on function public.set_trainer_discovery_prompt(text) from public, anon;
grant execute on function public.get_trainer_discovery_prompt() to authenticated;
grant execute on function public.set_trainer_discovery_prompt(text) to authenticated;
