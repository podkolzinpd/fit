-- YAFIT-288: история как спортивная хроника.
-- Карточка получает feedback и вычисляемый PR тем же постраничным запросом,
-- поэтому браузер не дочитывает всю историю и не делает N+1 запросов.
    ...
    when compared.input_kind = 'strength' then
      (
        ...
        and (
          ...
          or compared.best_weight_kg > compared.prior_best_weight_kg
        )
      )
      or
      (
        ...
        and (
          ...
          or compared.best_weight_reps > compared.prior_best_weight_reps
        )
      )
    else
      ...
      and (
        ...
        or compared.primary_value > compared.prior_primary_value
      )
  end), false)
  from compared;
$$;

comment on function public.workout_has_personal_record(uuid) is
  'Internal confirmed-only PR flag for a completed workout, compared with earlier completed workouts.';

revoke all on function public.workout_has_personal_record(uuid)
...
comment on function public.list_workouts(date, date, uuid, integer, integer) is
  'Accessible paginated workouts, newest first, with compact chronicle facts and confirmed-only PR.';
