...
revoke all on app_private.training_summary_generation_guards from public, anon, authenticated;
...
begin
  if coalesce(current_setting('request.jwt.claim.role', true), '') <> 'service_role'
    and not public.can_access_client(p_client_id)
  then
...
revoke all on function public.claim_training_summary_generation(uuid, date, date, text, uuid, integer, integer) from public, anon, authenticated;
revoke all on function public.complete_training_summary_generation(uuid, date, date, text, uuid, jsonb) from public, anon, authenticated;
revoke all on function public.fail_training_summary_generation(uuid, date, date, text, uuid, text, jsonb) from public, anon, authenticated;
grant execute on function public.claim_training_summary_generation(uuid, date, date, text, uuid, integer, integer) to service_role;
grant execute on function public.complete_training_summary_generation(uuid, date, date, text, uuid, jsonb) to service_role;
grant execute on function public.fail_training_summary_generation(uuid, date, date, text, uuid, text, jsonb) to service_role;
