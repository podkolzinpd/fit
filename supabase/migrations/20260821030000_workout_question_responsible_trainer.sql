-- Вопрос относится к ответственному тренеру конкретной тренировки.
-- Для назначенной тренировки это её автор; для самостоятельной тренировки
-- клиента — основной тренер карточки. Ранее функция всегда требовала
-- clients.trainer_id, хотя UI уже использует этот ответственный контракт.
