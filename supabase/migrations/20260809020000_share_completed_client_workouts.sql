-- Подключённый тренер видит завершённую историю, которую клиент записал сам,
-- но авторство мутаций не меняется: редактировать и удалять такую тренировку
-- по-прежнему может только клиент. Назначения других тренеров остаются скрыты.
...
returns table (
  id uuid, client_id uuid, client_name text, created_by uuid, workout_date date,
...
begin
  ...
  with paged_workouts as materialized (
    select
      ...
      workout.trainer_review, workout.client_comment, workout.version,
  ...
  select
    workout.id, workout.client_id, workout.client_name, workout.created_by,
