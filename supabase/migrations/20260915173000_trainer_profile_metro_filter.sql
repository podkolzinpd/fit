drop function if exists public.list_public_trainer_profiles_page(text, text, text, text, boolean, integer, integer);
...
create index if not exists trainer_profiles_catalog_metro_idx
  on public.trainer_professional_profiles using gin ((published_data->'metroStationIds'))
