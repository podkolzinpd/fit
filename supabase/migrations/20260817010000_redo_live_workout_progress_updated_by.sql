-- YAFIT-299: переналожение updated_by-контракта поверх текущего main.
--
-- Байт-в-байт повтор миграций 20260811010000 и 20260811020000 (PR #363, #365),
-- откаченных компенсационной 20260811030000_restore_post_361_contract.sql в рамках
-- YAFIT-279 (коллатеральный откат из-за независимого сломавшего прод PR).
-- Ничего из затронутых функций/analytics.client_overview не менялось миграциями
-- 20260814..20260816, поэтому повтор безопасен и не требует адаптации.
--
-- Новая timestamped-миграция, не редактирование истории — тот же принцип,
-- что применялся при самом откате (PR #368).
