...
} from "../../../services/api/src/legacy-workout-parser/extracted-workout.ts"
...
type Exercise = WorkoutParserExercise
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const system = body.systemCatalog.flatMap((raw): Exercise[] => {
    ...
    ].join("\n") : workoutExtractionPrompt(body.text)
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      ...
      try {
        ...
        const modelValue = JSON.parse(payload.result?.alternatives?.[0]?.message?.text ?? "")
        const result = goalMode
          ? validateGoal(modelValue, catalog, metrics ?? [])
          : matchWorkoutExtraction(validateWorkoutExtraction(modelValue), catalog)
