...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const prompt = ["Разбери всё описание тренировки целиком. Внутри текста может быть несколько упражнений подряд без переносов: самостоятельно раздели их по названиям и верни отдельный item для каждого. Верни упражнения в исходном порядке. Не объединяй два упражнения в один item и не додумывай отсутствующие значения. Учитывай спортивные синонимы и ошибки распознавания речи. Выбирай exerciseRef только из каталога. Верни только JSON по схеме items/unmatched.", "Текст: " + body.text, "Каталог: " + JSON.stringify(catalog.map(({ ref, name, inputKind }) => ({ ref, name, inputKind })))] .join("\n")
