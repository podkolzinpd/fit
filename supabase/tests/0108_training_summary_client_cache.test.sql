...
select ok(
  not has_function_privilege('anon', 'public.publish_cached_training_summary_for_client(uuid,date,date,text,text)', 'EXECUTE'),
  'anonymous actors cannot publish an internal cache directly'
);
select ok(
...
select throws_ok(
  $$select public.publish_cached_training_summary_for_client('a8000000-0000-4000-8000-000000000010', '2026-08-01', '2026-08-31', 'training-progress-v13', 'shared-fp')$$,
  '42501', null, 'authenticated role cannot execute the backend-only cache publisher'
);
reset role;
