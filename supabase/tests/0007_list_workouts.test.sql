...
select is((select id from public.list_workouts(null, null, null, 1, 1)), '7c000000-0000-4000-8000-000000000001'::uuid, 'newest-first aggregate offset returns the next stable row');
...
-- Блоки упражнений: существующие бэкфиллятся как single, list возвращает block_type.
select is(
  (select item->>'block_type' from public.list_workouts(null, null, null), jsonb_array_elements(exercises) as item where id = '7c000000-0000-4000-8000-000000000001' limit 1),
  'single',
  'list_workouts returns block_type (single by default)'
);
-- save_workout сохраняет суперсет-блок: два упражнения с одним blockId/blockType.
