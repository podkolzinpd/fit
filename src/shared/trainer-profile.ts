...
// 2026-09-22: тренер указал, что реабилитация (после травм, требует
// медицинское образование) и адаптивная физкультура (работа с ОВЗ, требует
// физкультурное образование) — разные специализации, поэтому старый общий
// пункт каталога разделили на REHAB_SPECIALTY/ADAPTIVE_PE_SPECIALTY ниже.
const REHAB_SPECIALTY = 'Реабилитация после травм и операций'
const ADAPTIVE_PE_SPECIALTY = 'Адаптивная физическая культура (для людей с особенностями здоровья)'
...
// Уже сохранённые анкеты со старым объединённым значением не мигрируются
// автоматически — нет данных, какое из двух направлений реально подходит
// тренеру, а приписать оба означало бы повторить ту же неточность, ради
// которой это разделили. SpecialtyChecklist показывает такую строку
// отдельной удаляемой пометкой (не теряет её молча, но и не даёт больше
// выбрать через чекбокс) — см. использование ниже в SpecialtyChecklist.tsx.
export const LEGACY_REHAB_ADAPTIVE_SPECIALTY = 'Реабилитация и адаптивная физкультура (после травм, ограничения по здоровью)'

// Каталожный поиск по любому из двух новых направлений должен по-прежнему
// находить тренера со старым значением — иначе он молча выпадает из
// результатов, пока сам не обновит анкету. Расширяем набор точных значений,
// которые репозитории (Supabase RPC p_specialties / Yandex specialty=)
// отправляют на бэкенд для поиска — сам бэкенд по-прежнему делает простое
// точное совпадение, ничего не знает про разделение направлений.
export function expandSpecialtiesForCatalogSearch(specialties: string[]): string[] {
  return specialties.includes(REHAB_SPECIALTY) || specialties.includes(ADAPTIVE_PE_SPECIALTY)
    ? [...specialties, LEGACY_REHAB_ADAPTIVE_SPECIALTY]
    : specialties
}
