...
export const ORIGINAL_SEARCH_ALIASES: Readonly<Record<string, readonly string[]>> = {
  ...
  'fedb-machine-bench-press': ['горизонтальный жим в тренажере'],
  ...
  'push-ups': ['отжимашки'],
  dips: ['брусья'],
  'pec-deck': ['бабочка', 'пек дек', 'сведение рук в тренажере бабочка', 'сведение рук в тренажере'],
  ...
  'fedb-cable-rope-overhead-triceps-extension': ['разгибание рук вверх на блоке'],
  plank: ['планка на локтях'],
  crunches: ['пресс', 'скручивания на пресс'],
  'fedb-decline-crunch': ['скручивания на наклонной скамье'],
...
// picker duplicates inherit exact phrases; visible variants keep their own
// match. Nothing here upgrades generated hints to exact aliases.
export const SEARCH_ALIASES: Readonly<Record<string, readonly string[]>> = (() => {
  ...
  for (const [ref, target] of Object.entries(EXERCISE_PICKER_REPLACEMENTS)) {
