...
describe('selectable exercise catalog', () => {
  ...
  it('retires exactly 83 roots without losing any historical ref or the four used exercises', () => {
    ...
    expect(selectable).toHaveLength(904 - 16 - 83)
    ...
    expect(['core', 'uncommon', 'rare', 'formats'].map((section) => roots.filter((exercise) => exerciseCatalogSection(exercise) === section).length)).toEqual([80, 515, 137, 7])
  })
