...
export function AssistantWorkoutDraftSurface({
  ...
  return <div className="assistant-flow-card assistant-program-draft assistant-workout-draft" aria-label={`${mode === 'collecting' ? 'Черновик' : 'Разбор'} тренировки ${clientName}`}>
    <div className="assistant-workout-draft-content">
      <header className="assistant-workout-header"><span className="assistant-workout-heading"><small>{mode === 'collecting' ? 'Черновик тренировки' : 'Тренировка'}</small><strong>{clientName}</strong></span><span className="assistant-flow-status">{parsing ? 'Распознаю…' : mode === 'collecting' ? 'Диктовка' : 'Проверка'}</span></header>
      <div className="assistant-workout-meta"><label><span>Дата</span><input type="date" value={workoutDate} onChange={(event) => onDateChange(event.target.value)} /></label><label><span>Время</span><input type="time" value={startTime} onChange={(event) => onTimeChange(event.target.value)} /></label></div>
