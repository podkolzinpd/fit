...
export function AssistantWorkoutDraftSurface({
  ...
  return <div className="assistant-flow-card assistant-program-draft assistant-workout-draft" aria-label={`${mode === 'collecting' ? 'Черновик' : 'Разбор'} тренировки ${clientName}`}>
    ...
    </div>{mode === 'confirm' && !unmatched.length && result.items.length > 0 && <button type="button" onClick={onSave} disabled={saving || saved}>{saved ? 'Тренировка сохранена' : saving ? 'Сохраняю…' : 'Сохранить тренировку'}</button>}</div>}
    ...
    {!saved && <div className="assistant-flow-actions">{mode === 'collecting' && <button type="button" onClick={onFinish} disabled={!canFinish}>Проверить и сохранить</button>}<button type="button" className="assistant-action-cancel" onClick={onCancel}>Отменить сценарий</button></div>}
  </div>
}
