...
describe('assistant program draft saving', () => {
  it('builds planned workout drafts with stable request ids for a retry', () => {
    const input = [{ title: 'Силовая A', day: 'Понедельник', exercises: ['Жим штанги лёжа'] }]
    ...
    expect(first?.[0]?.exercises[0]).toMatchObject({ ref: 'barbell-bench-press', sets: [{ position: 0 }] })
  ...
  it('blocks confirmation until every exercise maps unambiguously to the catalog', () => {
    expect(programWorkoutDrafts(
      ...
      [{ title: 'Силовая A', day: 'Понедельник', exercises: ['Несуществующее упражнение'] }],
