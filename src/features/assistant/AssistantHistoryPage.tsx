...
export function AssistantHistoryPage() {
  ...
  return <main className="assistant-page">
    ...
    <section ref={threadRef} className="assistant-thread" aria-label="Диалог с ассистентом">
      ...
      {error && <div className="assistant-message-error" data-message-kind="error" role="alert"><span>{error}</span>{failedTurn && <button type="button" onClick={() => void send(undefined, failedTurn)} disabled={sending}>Повторить отправку</button>}</div>}
    </section>
