...
export function AssistantHistoryPage({ backend = supabaseAssistantBackend }: {
  ...
  return <main className={`assistant-page${programCollecting ? ' assistant-program-collecting' : ''}`}>
    ...
    <form className="assistant-composer" autoComplete="off" onSubmit={(event) => { event.preventDefault(); void send() }}>
      ...
      <textarea ref={composerInputRef} id="assistant-history-message" name="assistant-prompt" autoComplete="off" rows={1} value={text} onChange={(event) => setText(event.target.value)} onInput={(event) => { event.currentTarget.style.height = 'auto'; event.currentTarget.style.height = `${Math.min(event.currentTarget.scrollHeight, 112)}px` }} placeholder={programEnabled ? 'Напишите, чем помочь' : 'Опишите тренировку'} disabled={!conversationId || readOnly || sending || voiceActive} />
      <VoiceInputButton
        variant="icon"
        source="assistant"
...
function AssistantWorkoutDraftCard({ mode, payload, timezone, catalog, parseWorkout, storageKey, onCancel, onApply, onSaved, onFinish }: { mode: 'collecting' | 'confirm'; payload: WorkoutDraftPayload; timezone?: string; catalog: AssistantCatalog; parseWorkout: AssistantWorkoutParser; storageKey?: string; onCancel: () => void; onApply: (input: object) => Promise<void>; onSaved: () => void; onFinish?: () => void }) {
  ...
  function chooseExercise(sourceText: string, ref: string) {
    ...
    if (!exercise) return
    setResult((current) => current ? resolveWorkoutParseSource(current, sourceText, ref) : current)
  }
  function updateMetrics(sourceText: string, patch: { setCount?: number; reps?: number; weightKg?: number }) {
    setResult((current) => current ? updateWorkoutParseMetrics(current, sourceText, patch) : current)
  }
  function removeExercise(sourceText: string) {
