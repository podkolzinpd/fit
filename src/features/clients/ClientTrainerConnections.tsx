...
export function ClientTrainerConnections({ clientId }: { clientId: string }) {
  ...
  const removeTrainer = useMutation({ mutationFn: (trainerId: string) => invitationsRepository.removeTrainer(clientId, trainerId), onSuccess: async () => queryClient.invalidateQueries({ queryKey: ['client-trainers', clientId] }) })
