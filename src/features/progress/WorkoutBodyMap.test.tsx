...
describe('Home and Progress workout map', () => {
  ...
  it('keeps the selected workout, mode, zone and dates through Home → Progress → source → back without AI', async () => {
    ...
    const link = screen.getByRole('link', { name: 'Разобрать нагрузку' })
    expect(link).toHaveAttribute('href', workoutMapLink(workout, 'chest'))
    await user.click(link)
    expect(document.querySelector('details')).toHaveAttribute('open')
    expect(screen.getByText(/10 августа 2026 г. · одна завершённая тренировка/)).toBeVisible()
    expect(screen.getByText('Жим лёжа: 2 подхода')).toBeVisible()
    await user.click(screen.getByRole('link', { name: 'Открыть исходную тренировку' }))
    ...
    expect(screen.getByText(/одна завершённая тренировка/)).toBeVisible()
  })
  ...
  it('shows recorded work without a large empty figure when no zone is mapped', () => {
    ...
    expect(screen.getByText(/Записи кардио: 1/)).toHaveTextContent('Без определённой зоны: 1 подход')
  })
  ...
  it('keeps additional zones accessible and switches the selected zone with the side', async () => {
    ...
    await user.click(screen.getByText('Ещё зоны · 2'))
    await user.click(screen.getByRole('button', { name: 'Верх спины: 1 подход' }))
    expect(screen.getByRole('link', { name: 'Разобрать нагрузку' })).toHaveAttribute('href', workoutMapLink(workout, 'upper_back'))
  ...
  it('supports keyboard and swipe selection on the full map', () => {
    ...
    const region = screen.getByRole('button', { name: 'Грудь. Доля подходов с определённой зоной: 67%' })
  ...
  it('recovers from a failed request, rejects another client and explicitly clears a workout scope', async () => {
    ...
    expect(screen.getByText(/В выбранной зоне больше нет/)).toBeVisible()
    expect(screen.getByText(/В этой тренировке пока нет подтверждённых подходов/)).toBeVisible()
    view.rerender(panel({ workouts: [workout] }))
    await user.click(screen.getByRole('button', { name: 'Перейти к карте за период' }))
    expect(screen.getByText('Карта тела · выбранный период')).toBeVisible()
    expect(screen.getByText('1 августа 2026 г. — 31 августа 2026 г.')).toBeVisible()
    await user.click(screen.getByText('Карта тела · выбранный период'))
    expect(document.querySelector('details')).not.toHaveAttribute('open')
  })
})
