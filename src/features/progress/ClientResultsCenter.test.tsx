...
describe('result center and weekly work', () => {
  it('keeps detail collapsed and retains an earlier PR despite later decline', async () => {
    ...
    await user.click(screen.getByText('Все результаты и рекорды'))
    ...
    expect(screen.getByText('Меньше прошлого результата')).toBeVisible()
    expect(screen.getByText('Больше прошлого результата')).toBeVisible()
    const pr = screen.getByText('Личный рекорд').closest('article')!
    expect(pr).toHaveTextContent('Ранее: 40 кг')
  ...
  it('separates fixed-weight reps and volume and explains actual sets', async () => {
    ...
    expect(screen.getByText('Личный рекорд').closest('article')).toHaveTextContent('Ранее: 10 повт.')
    await user.selectOptions(screen.getByLabelText('Показатель'), 'volume')
    const latest = screen.getAllByRole('article')[0]!
    await user.click(within(latest).getByText('Из чего сложился объём'))
  ...
  it('does not silently replace a missing exercise filter with all exercises', () => {
    ...
    expect(screen.getByRole('status')).toHaveTextContent('Выбранный фильтр больше недоступен')
  ...
  it('keeps load failure actionable and avoids a fabricated empty result', async () => {
    ...
    expect(screen.queryByText('Подтверждённых результатов с такими условиями пока нет.')).toBeNull()
  })
  it('shows partial weeks and confirmed work after explicit disclosure', async () => {
    ...
    await userEvent.setup().click(screen.getByText('Подходы по неделям'))
    expect(await screen.findAllByText('Неполная неделя в выбранном периоде.')).toHaveLength(2)
    expect(screen.getByText('10 августа 2026 г. — 12 августа 2026 г.')).toBeVisible()
    expect(screen.getAllByText('По зонам: 1. Кардио: 0. Без определённой зоны: 0.')).toHaveLength(3)
  })
  it('recalculates records after editing and deleting their source without preserving a stale PR', () => {
    ...
    expect(down).toHaveTextContent('Ранее: 40 кг')
  })
  it('distinguishes pending history from an empty period and rejects unknown metrics', () => {
    ...
    expect(screen.getByText('Подтверждённых результатов с такими условиями пока нет.')).toBeVisible()
    unmount()
  ...
  it('expands older weeks and distinguishes pending, failed and future-only ranges', async () => {
    ...
    await user.click(screen.getByText('Подходы по неделям'))
    ...
    expect(screen.getByRole('status')).toHaveTextContent('Собираем недели')
    ...
    expect(screen.getByText('За выбранные даты пока нет доступных недель.')).toBeVisible()
  })
