...
async function openPro(user = userEvent.setup()) {
  const tab = await screen.findByRole('tab', { name: 'ПРО' })
  if (tab.getAttribute('aria-selected') !== 'true') await user.click(tab)
}

async function openOverview(user = userEvent.setup()) {
...
describe('Training summary card states', () => {
  ...
  it('opens a legacy linked measurements target directly inside Overview', async () => {
    ...
    render(<MemoryRouter initialEntries={['/me/progress?view=pro#measurements']}><QueryClientProvider client={cache}>
  ...
  it('creates the first client analysis only after an explicit click', async () => {
    ...
    const create = await screen.findByRole('button', { name: 'Создать анализ' })
    expect(repositories.generate).not.toHaveBeenCalled()
    await user.click(create)
    await waitFor(() => expect(repositories.generate).toHaveBeenCalledWith(
      'client-1', '2026-07-21', '2026-08-20', true, 'create',
    ))
  })
  ...
  it('accepts a short history, a long exercise name and no client goal without leaking technical text', async () => {
    ...
    expect(detailsTrigger.closest('.progress-overview-panel')).not.toBeNull()
    ...
    expect(within(details).queryByRole('heading', { name: 'На следующей тренировке' })).toBeNull()
  ...
  it('turns the client summary into a factual period, goal and upcoming-plan story', async () => {
    ...
    expect(goal).toHaveTextContent('81,5 кг')
    expect(goal).toHaveTextContent('85 кг')
    expect(screen.queryByRole('heading', { name: 'Ты приближаешься к цели' })).toBeNull()
    expect(goal).toHaveClass('compact')
    await user.click(within(goal).getByRole('link', { name: 'Подробнее в ПРО' }))
  ...
  it('combines the main result with the goal and shows no more than two criteria at first', async () => {
    ...
    expect(goal).toHaveClass('client-progress-goal-story', 'standalone', 'compact')
    ...
    await user.click(within(goal!).getByRole('link', { name: 'Подробнее в ПРО' }))
    const detailedGoal = screen.getByRole('region', { name: 'Твоя цель' })
    expect(within(detailedGoal).getByText('Регулярность тренировок')).toBeVisible()
    expect(within(detailedGoal).getByText('Талия')).not.toBeVisible()
    await user.click(within(detailedGoal).getByText('Все показатели · 1'))
    expect(within(detailedGoal).getByText('Талия')).toBeVisible()
    await user.click(within(detailedGoal).getByText('Все показатели · 1'))
    expect(within(detailedGoal).getByText('Талия')).not.toBeVisible()
  })
  ...
  it('hides low-value legacy fallback and keeps the structured detailed-analysis sections', async () => {
    ...
    await screen.findByText('Здесь появится карта нагрузки после тренировки.')
    await openOverview(user)
  ...
  it('keeps the trainer analysis read-only on mount and refreshes only after an explicit click', async () => {
    const user = userEvent.setup()
    const updated = {
      ...trainerSummary,
      id: 'summary-2',
    ...
    expect(repositories.generate).not.toHaveBeenCalled()
    expect(repositories.listForTrainer).toHaveBeenCalledTimes(1)
    ...
    expect(repositories.generate).toHaveBeenCalledWith(
      'client-1', expect.any(String), expect.any(String), true, 'manual_refresh',
    )
  ...
  it('keeps trainer signals collapsed and never renders the block in the client card', async () => {
    ...
    repositories.listForClient.mockResolvedValue([publishedSummary])
    render(<ClientTrainingSummaryCard clientId="client-1" />, { wrapper: wrapper(queryClient()) })
    expect(await screen.findByRole('region', { name: 'Твоя цель' })).toBeVisible()
    expect(document.querySelector('.client-progress-next-step')).toBeNull()
    expect(screen.queryByText('Для тренера')).toBeNull()
  })

  it('keeps the trainer card after an explicit refresh error and retries only on demand', async () => {
    const user = userEvent.setup()
    repositories.firstCompletedWorkoutDate.mockResolvedValue(localDate('2026-07-20'))
    repositories.listForTrainer.mockResolvedValue([trainerSummary])
  ...
  it('keeps trainer progress visible and retries a failed body load request', async () => {
    ...
    expect(await screen.findByText('Здесь появится карта нагрузки после тренировки.')).toBeVisible()
  ...
  it('keeps the current client analysis and exposes an explicit refresh error', async () => {
    repositories.firstCompletedWorkoutDate.mockResolvedValue(localDate('2026-07-20'))
    repositories.listForClient.mockResolvedValue([publishedSummary])
  ...
  it('keeps the first-record action and goal visible without a saved AI analysis', async () => {
    ...
    repositories.listForClient.mockResolvedValue([])
    render(<ClientTrainingSummaryCard clientId="client-1" />, { wrapper: wrapper(queryClient()) })
    expect(await within(await screen.findByRole('region', { name: 'Твоя цель' })).findByText('Цель пока не указана')).toBeVisible()
  ...
  it('lets the client inspect only the period load percentage for a body zone', async () => {
    const user = userEvent.setup()
    repositories.firstCompletedWorkoutDate.mockResolvedValue(localDate('2026-07-20'))
    repositories.listForClient.mockResolvedValue([publishedSummary])
    repositories.workouts.mockResolvedValue([{
    ...
    expect(loadDetail).not.toHaveAttribute('data-copy-source')
    expect(screen.queryByRole('button', { name: /Показать .* упражн/ })).toBeNull()
  ...
  it('requests a new analysis explicitly and preserves saved text after a force error', async () => {
    ...
    expect(within(dialog).getByRole('heading', { name: 'Почему' })).toBeVisible()
    expect(repositories.generate).toHaveBeenLastCalledWith('client-1', '2026-07-21', '2026-08-20', true, 'manual_refresh')
  })

  it('does not generate on period switch or remount', async () => {
    repositories.firstCompletedWorkoutDate.mockResolvedValue(localDate('2026-01-01'))
    repositories.listForClient.mockResolvedValue([publishedSummary])
    const cache = queryClient()
    const user = userEvent.setup()
    const view = render(<ClientTrainingSummaryCard clientId="client-1" />, { wrapper: wrapper(cache) })

    await screen.findByRole('button', { name: '3 месяца' })
    await user.click(screen.getByRole('button', { name: '3 месяца' }))
    expect(repositories.generate).not.toHaveBeenCalled()

    view.unmount()
    render(<ClientTrainingSummaryCard clientId="client-1" />, { wrapper: wrapper(queryClient()) })
    await screen.findByRole('button', { name: '1 месяц' })
    expect(repositories.generate).not.toHaveBeenCalled()
  })
  ...
  it('keeps an in-flight forced month analysis scoped when the user switches to three months', async () => {
    ...
    expect(document.querySelector('.progress-analysis-preview')).toHaveTextContent('Обновлён 20.08.2026')
  ...
  it('shows the selected map dates instead of dates borrowed from an old AI summary', async () => {
    ...
    expect(document.querySelector('.progress-analysis-preview')).toHaveTextContent('21 июня 2026 г. — 20 июля 2026 г.')
  })
  ...
  it('marks new workouts only when a completed workout in the selected period is newer than the analysis', async () => {
    ...
    await userEvent.setup().click(screen.getByRole('button', { name: 'Обновить анализ' }))
    await waitFor(() => expect(repositories.generate).toHaveBeenLastCalledWith(
      'client-1', '2026-07-21', '2026-08-20', true, 'new_workout',
    ))
    ...
    expect(repositories.generate).toHaveBeenCalledTimes(1)
  })
