...
export function MeasurementProgressSection({
  ...
  return <section className="client-progress-measurements-story" id={role === 'client' ? 'measurements' : 'progress-measurements'} aria-labelledby={`${role}-progress-measurements-title`}>
    <header>
      <div>{!compact && <span>Измерения</span>}<h3 id={`${role}-progress-measurements-title`}>{compact ? "Замеры" : "Тренд по значениям"}</h3></div>
      {!management && <Link className="link" to={measurementLink}>{role === 'client' ? 'Управление' : 'Все замеры'}</Link>}
    </header>
    {loading ? <p className="measurement-story-state" role="status">Собираем историю замеров…</p>
      ...
      : !selected ? <div className="measurement-story-empty"><p>{compact ? "Добавь первый замер, чтобы видеть изменения." : "Добавь первый замер — здесь появятся значения, свежесть и график."}</p>{!management && <Link className="link" to={measurementLink}>Добавить замер</Link>}</div>
      : <>
        ...
        <div className="measurement-story-current" data-fact-id={selected.factId}>
          <div><span>{compact ? "На конец периода" : "Сейчас"}</span><strong>{observationValue(selected, currentObservation)}</strong></div>
          <div><span>{selected.delta === null ? (compact ? 'Пока без сравнения' : 'Динамика периода') : formatMeasurementDelta(selected.delta, selected.unit)}</span><small>{currentObservation ? formatLocalDate(currentObservation.date) : 'Нет замера'}</small></div>
        </div>
        <ProgressChart
          entries={[...entries]}
        ...
        />
        {compact && selected.goalGuide && <p className="muted">{selected.goalGuide.label}</p>}
        {compact ? <details className="measurement-story-details"><ProgressDetailsSummary>Подробности замеров</ProgressDetailsSummary>        <dl className="measurement-story-facts">
        ...
        {selected.hasNewerValueAfterPeriod && <p className="measurement-story-after-period">Есть более свежий замер.</p>}
