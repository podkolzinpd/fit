...
export function ProfilePage() {
  ...
  return <Page title="Профиль"><form className="stack" onSubmit={(event) => void submit(event)}><Field label="Имя"><input name="firstName" defaultValue={actor?.firstName ?? ''} /></Field><Field label="Фамилия"><input name="lastName" defaultValue={actor?.lastName ?? ''} /></Field><Field label="Часовой пояс"><input name="timezone" defaultValue={actor?.timezone ?? 'Europe/Moscow'} /></Field>{update.error && <p className="error">{update.error.message}</p>}{saved && <p className="success">Сохранено</p>}<button>Сохранить</button></form><div className="menu"><Link to="/exercises">Управление упражнениями</Link></div><button className="danger secondary wide" onClick={() => void logout()}>Выйти</button></Page>
}
