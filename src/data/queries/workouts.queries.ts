...
const rootColumns = 'id,trainer_id,client_id,created_by,started_by,completed_by,workout_date,start_time,end_time,started_at,completed_at,status,notes,trainer_review,trainer_reaction,trainer_review_author_id,trainer_reviewed_at,client_comment,session_rpe,wellbeing,discomfort,feedback_submitted_at,client_question,client_question_asked_at,client_question_resolved_at,version,stage_id'
...
export const workoutQueries = {
  ...
  recordPlannedResult: (draft: WorkoutDraft) => supabase.rpc('record_planned_workout_result', {
  ...
  removeLiveExercise: (workoutId: string, exerciseId: string, version: number) => supabase.rpc('remove_live_exercise', {
