...
export const pushNotificationsQueries = {
  upsertSubscription: (userId: string, subscription: BrowserPushSubscription) => getSupabaseClient()
  ...
  deleteSubscriptionByEndpoint: (userId: string, endpoint: string) => getSupabaseClient()
    .from('push_subscriptions').delete().eq('user_id', userId).eq('endpoint', endpoint),
  getSubscriptionByEndpoint: (userId: string, endpoint: string) => getSupabaseClient()
    .from('push_subscriptions').select('user_id').eq('user_id', userId).eq('endpoint', endpoint).maybeSingle(),
  getPreference: (userId: string, kind: string) => getSupabaseClient()
    .from('notification_preferences')
    .select('enabled')
    .eq('user_id', userId)
    .eq('kind', kind)
    .maybeSingle(),
  setPreference: (userId: string, kind: string, enabled: boolean) => getSupabaseClient()
    .from('notification_preferences')
    .upsert({ user_id: userId, kind, enabled }, { onConflict: 'user_id,kind' }),
  sendTestPush: (endpoint: string) => getSupabaseClient().rpc('send_test_push_notification', { p_endpoint: endpoint }),
}
