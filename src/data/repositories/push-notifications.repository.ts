...
import { CHAT_MESSAGE_KIND, pushNotificationsQueries, WORKOUT_REMINDER_KIND, WORKOUT_SCHEDULED_KIND } from '../queries/push-notifications.queries'
...
export { CHAT_MESSAGE_KIND, WORKOUT_REMINDER_KIND, WORKOUT_SCHEDULED_KIND }
...
export const pushNotificationsRepository = {
  async status(userId: string): Promise<NotificationStatus> {
    ...
    const [state, reminderPreference, scheduledPreference, chatPreference] = await Promise.all([
      ...
      pushNotificationsQueries.getPreference(userId, CHAT_MESSAGE_KIND),
    ])
    ...
    if (chatPreference.error) throw repositoryError(chatPreference.error)
    return {
      state,
      ...
      chatMessageEnabled: chatPreference.data?.enabled ?? true,
    }
  },
