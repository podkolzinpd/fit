...
async function get(id: string): Promise<Workout> {
  ...
  return {
    ...
    favoriteTitle: root.data.favorite_title,
    ...
    activeCaloriesKcal: root.data.active_calories_kcal ?? null,
...
export const workoutsRepository = {
  ...
  async removeLiveExercise(workout: Workout, exerciseId: string): Promise<number> {
    const result = await workoutQueries.removeLiveExercise(workout.id, exerciseId, workout.version)
    if (result.error) throw repositoryError(result.error)
    return result.data
  },
