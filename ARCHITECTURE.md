...
- `auth_user_id` связывает карточку с клиентом-владельцем; `client_trainers` предоставляет доступ нескольким тренерам. Legacy `trainer_id` временно сохраняется как автор/совместимый tenant key до перевода всех aggregate RPC.
...
- Приватная заметка карточки остаётся тренерской. Комментарий упражнения является общим полем пространства клиента; клиент-владелец и подключённые тренеры редактируют его по тем же membership-правилам, что workout aggregate.
