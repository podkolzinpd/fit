...
- Перед обращением к модели оба серверных контура строят один детерминированный
  fingerprint и получают право генерации через private DB guard. Он возвращает
  кеш при неизменных данных, блокирует параллельный дубль, вводит 30-минутный
  cooldown после ошибки и бюджеты один вызов на период в сутки / три на клиента
  в сутки. В модель уходит агрегат не больше 20 000 символов; один fingerprint
  создаёт максимум один provider call и ответ не длиннее 1 000 токенов.
