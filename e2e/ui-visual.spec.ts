...
test('trainer Progress and measurements form keep their visual baselines in both themes', async ({ page }, testInfo) => {
  ...
  await expect(lightDetails.getByRole('heading', { name: 'Почему' })).toBeVisible()
  await expect(lightDetails.getByRole('heading', { name: 'На следующей тренировке' })).toBeVisible()
  ...
  await expect(darkDetails.getByRole('heading', { name: 'Почему' })).toBeVisible()
  await expect(darkDetails.getByRole('heading', { name: 'На следующей тренировке' })).toBeVisible()
