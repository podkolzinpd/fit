...
test('exercise catalog preview keeps search, media and detail transitions usable', async ({ page }) => {
  ...
  const textOnlyDetail = page.getByRole('dialog')
  await expect(textOnlyDetail.locator('.exercise-image, img, video')).toHaveCount(0)
  await expect(textOnlyDetail.getByText('Расстояние', { exact: true })).toBeVisible()
  await textOnlyDetail.locator('button.secondary').click()
