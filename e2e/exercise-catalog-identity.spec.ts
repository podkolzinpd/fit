...
test('exercise catalog preview keeps search, media and detail transitions usable', async ({ page }) => {
  ...
  await expect(photoResult.getByText('Горизонтальный велотренажёр (Тренажёр)')).toBeVisible()
  await expect(photoResult.locator('video')).toHaveCount(0)
  await expect(photoResult.locator('.exercise-image-frame-start')).toBeVisible()
  await photoResult.click()
  const photoDetail = page.getByRole('dialog')
  await expect(photoDetail.locator('.exercise-image-technique img')).toHaveCount(2)
  await expect(photoDetail.getByText('Расстояние', { exact: true })).toBeVisible()
  await photoDetail.locator('button.secondary').click()
