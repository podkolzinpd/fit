...
test('linked client sees only the published client progress view', async ({ page }) => {
  ...
  await expect(page.getByText('Добавь критерии, чтобы видеть изменение каждого показателя.')).toBeVisible()
  await expect(page.locator('.personal-workout-result')).not.toHaveText('Рабочий вес: 72 → 75 кг · +4%')
  await expect(page.getByRole('link', { name: 'Настроить оценку' })).toHaveAttribute('href', '/me/goal')
