...
test('linked client sees only the published client progress view', async ({ page }) => {
  ...
  await expect(nextStep.getByText('Черновик · правила', { exact: true })).toBeVisible()
  ...
  await nextStep.getByRole('button', { name: 'Отклонить' }).click()
  await expect(nextStep.getByRole('heading', { name: 'Рекомендация отклонена' })).toBeVisible()
  await nextStep.getByRole('button', { name: 'Вернуть черновик' }).click()
