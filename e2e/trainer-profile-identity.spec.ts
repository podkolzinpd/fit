...
test('trainer profile keeps the questionnaire compact and settings usable on a separate screen', async ({ page }) => {
  ...
  const liveAnimation = page.getByRole('switch', { name: 'Анимация упражнения', exact: true })
  ...
  const rest = page.getByRole('switch', { name: 'Показывать отдых', exact: true })
  const restBefore = await rest.isChecked()
  await rest.setChecked(!restBefore)
  await expect(rest).toBeChecked({ checked: !restBefore })
  await rest.setChecked(restBefore)

  const rpe = page.getByRole('switch', { name: 'Показывать RPE', exact: true })
  ...
  await page.getByRole('button', { name: 'Изменить данные' }).click()
  await expect(page.getByLabel('Имя', { exact: true })).toHaveValue(savedName)
  await page.getByRole('button', { name: 'Отмена' }).click()
