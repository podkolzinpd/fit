...
test('iPhone: client progress keeps one goal-aware LLM summary and compact running facts', async ({ page }) => {
  ...
  await expect(page.locator('.client-progress-card .ai-progress-hero')).toContainText('Главное за период')
...
test('iPhone: LLM regularity stays inside the single Progress summary at 390 px', async ({ page }) => {
  ...
  await expect(summary.locator('.ai-progress-hero')).toBeVisible()
  await expect(summary.getByText('Ритм тренировок', { exact: true })).toBeVisible()
