...
test('iPhone: поиск и фильтры каталога не перекрывают друг друга', async ({ page }) => {
  ...
  const technique = page.getByRole('button', { name: /Проиграть технику: Присед/ }).first()
  await expect(technique).toBeInViewport()
  await technique.click()
  await page.getByRole('button', { name: /Открыть технику: Присед/ }).first().click()
...
test('iPhone: тренер назначает интервалы, спортсмен подтверждает работу и восстановление', async ({ browser }, testInfo) => {
  ...
  try {
    ...
    await expect(trainer.getByText('Семенящий бег', { exact: true })).toBeVisible()
    await expect(trainer.getByRole('button', { name: 'Проиграть технику: Семенящий бег', exact: true })).toHaveCount(0)
    await expectNoHorizontalOverflow(trainer)
...
test('iPhone: client progress keeps one goal-aware LLM summary and compact running facts', async ({ page }) => {
  ...
  const mapDisclosure = page.locator('.client-body-map-disclosure')
  await expect(mapDisclosure).not.toHaveAttribute('open')
  await mapDisclosure.locator('summary').click()
  ...
  await expect(regularity.getByText('По неделям', { exact: true })).toBeVisible()
  await expect(regularity.getByText(/Активные: \d+ из \d+/)).toBeVisible()
  ...
  await expect(regularity.getByText('Тренировались', { exact: true })).toHaveCount(0)
  await expect(regularity.getByText('Без тренировок', { exact: true })).toHaveCount(0)
  ...
  await page.getByRole('tab', { name: 'ПРО' }).click()
  await page.getByText('Бег', { exact: true }).click()
...
for (const width of [320, 375]) {
  test(`iPhone: главное в Progress читается без переполнения на ${width} px`, async ({ page }) => {
    ...
    await expect(personal.getByRole('heading', { name: 'Лучшие результаты' })).toBeVisible()
...
for (const viewport of [{ width: 320, height: 700 }, { width: 375, height: 812 }, { width: 390, height: 844 }, { width: 430, height: 932 }]) {
  test(`iPhone: Progress тренера остаётся компактным на ${viewport.width} px`, async ({ page }) => {
    ...
    await expect(analysis.getByRole('list', { name: 'Завершённые тренировки по неделям' })).toBeVisible()
...
for (const viewport of [{ width: 320, height: 700 }, { width: 375, height: 812 }, { width: 390, height: 844 }, { width: 430, height: 932 }]) {
  test(`iPhone: client Progress keeps one compact result summary at ${viewport.width} px`, async ({ page }) => {
    ...
    await summary.getByText('Регулярность', { exact: true }).click()
    await expect(summary.getByRole('heading', { name: 'Тренировочный ритм' })).toBeVisible()
    await expect(summary.getByRole('list', { name: 'Завершённые тренировки по неделям' })).toBeVisible()
    ...
    await expect(summary.getByRole('heading', { name: 'Нагрузка по телу' })).toBeVisible()
    await expectCompactBodyMap(summary.locator('.body-progress-map'))
    ...
    await darkSummary.getByRole('tab', { name: 'ПРО' }).click()
...
test('iPhone: длинное название и 10 подходов не ломают форму и Live на 390 px', async ({ page }, testInfo) => {
  ...
  const fillNextSet = page.getByLabel('Фактический вес').nth(1)
  await expect(fillNextSet).toBeVisible()
  await expect(fillNextSet).toBeEditable()
  expect((await fillNextSet.boundingBox())!.height).toBeGreaterThanOrEqual(44)
  await expectNoHorizontalOverflow(page)
})

test('iPhone: одиночный отдых меняется только сверху и не блокирует подход после просрочки', async ({ page }, testInfo) => {
  await page.setViewportSize({ width: 390, height: 844 })
  const clientName = await createIsolatedClient(page, testInfo)
  await page.goto('/workouts/new')
  await selectClient(page, clientName)
  ...
  await page.getByLabel('Повторы, подход 2').fill('10')
  await page.getByRole('button', { name: '＋ Подход' }).click()
  await page.getByLabel('Вес, подход 3').fill('40')
  await page.getByLabel('Повторы, подход 3').fill('10')
  ...
  const dialog = page.getByRole('dialog', { name: 'Таймер отдыха' })
  const apply = dialog.getByRole('button', { name: 'Применить время · 1:30' })
  await expect(apply).toBeInViewport()
  await dialog.getByRole('option', { name: '00 минуты' }).click()
  await dialog.getByRole('option', { name: '02 секунды' }).click()
  ...
  await expect(secondSet).toHaveClass(/confirmed/)
  await expect(page.locator('.live-rest-trigger').filter({ hasText: /Отдых 0:0[12]/ })).toBeVisible()
  ...
  await expect(page.locator('.live-rest-trigger').filter({ hasText: /Отдых 0:1[4-7]/ })).toBeVisible()
  await page.reload()
  await expect(page.locator('.live-rest-trigger').filter({ hasText: /Отдых 0:1[2-7]/ })).toBeVisible()
  if (!await page.getByRole('dialog', { name: 'Таймер отдыха' }).isVisible()) await page.getByRole('button', { name: /^Таймер отдыха/ }).click()
...
test('iPhone: подходы Live разделены при полноширинных touch-зонах на 360 px', async ({ page }, testInfo) => {
  ...
  await expect(setRow.locator('.live-set-plan-caption')).toContainText('План · 40 кг × 10 повт.')
  await page.getByRole('button', { name: 'Ещё действия' }).click()
  await page.getByRole('menuitem', { name: 'Скрыть план' }).click()
  await expect(setRow.locator('.live-set-plan-caption')).toHaveCount(0)
  await expectNoHorizontalOverflow(page)
})
...
test('iPhone: частично завершённая тренировка помечена на 390 и 430 px', async ({ page }, testInfo) => {
  ...
  await expect(completedDetails.locator('.workout-history-set.confirmed').first()).toHaveCSS('background-color', 'rgba(0, 0, 0, 0)')
  await page.getByRole('button', { name: 'Другие действия с тренировкой' }).click()
  await expect(page.getByRole('menuitem', { name: 'Копировать тренировку' })).toBeVisible()
...
test('iPhone: составная цель настраивается вручную без горизонтального переполнения', async ({ page }) => {
  ...
  const configure = page.getByRole('button', { name: 'Настроить' })
  const automaticEvaluation = page.getByRole('switch', { name: 'Автоматическая оценка' })
  await expect(configure.or(automaticEvaluation)).toBeVisible()
  if (await configure.isVisible()) await configure.click()
  await automaticEvaluation.check()
...
test('iPhone: круговая использует отдых между упражнениями и между кругами', async ({ page }, testInfo) => {
  ...
  await confirmCurrentSet(page)
  await expect(page.locator('.circuit-round.done')).toHaveCount(1)
  await expect(page.locator('.live-rest-trigger').filter({ hasText: /Отдых 0:5[7-9]|Отдых 1:00/ })).toBeVisible()
  await expect(page.locator('.live-pinned .circuit-counter')).toHaveText('Круг 2 из 2')
  if (!await page.getByRole('dialog', { name: 'Таймер отдыха' }).isVisible()) await page.getByRole('button', { name: /^Таймер отдыха/ }).click()
  await page.getByRole('button', { name: 'Остановить отдых' }).click()
  await confirmCurrentSet(page)
