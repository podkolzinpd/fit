...
test('iPhone: client progress keeps one goal-aware LLM summary and compact running facts', async ({ page }) => {
  ...
  await expect(page.locator('.client-progress-card .client-progress-hero')).toBeVisible()
...
test('iPhone: client Progress keeps one compact result summary at 390 px', async ({ page }) => {
  ...
  await expect(summary.locator('.client-progress-hero')).toBeVisible()
...
for (const viewport of [{ width: 390, height: 844 }, { width: 430, height: 932 }]) {
  test(`iPhone: короткая история и длинное упражнение не ломают Progress на ${viewport.width} px`, async ({ page }) => {
    ...
    await page.goto('/me/progress')
    const summary = page.locator('.client-progress-card')
    await expect(summary.getByRole('heading', { name: 'Тяга верхнего блока обратным узким хватом в кроссовере с дополнительной рукоятью' })).toBeVisible()
    await expect(summary.getByText('+36%')).toBeVisible()
    await expect(summary.getByText('Рабочий вес: 50 → 68 кг')).toBeVisible()
