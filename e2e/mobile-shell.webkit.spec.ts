...
test('iPhone: client voice-first home сохраняет тренировку только себе, а Cancel профиля не мутирует данные на 390 px', async ({ page }, testInfo) => {
  ...
  await expectNoHorizontalOverflow(page)
  await expect(page.locator('.client-home-self-training .voice-action')).toHaveScreenshot('client-first-run-compact-actions.png', { animations: 'disabled', caret: 'hide' })
...
for (const viewport of [{ width: 320, height: 700 }, { width: 375, height: 812 }, { width: 390, height: 844 }, { width: 430, height: 932 }]) {
  test(`iPhone: client Progress keeps one compact result summary at ${viewport.width} px`, async ({ page }) => {
    ...
    await summary.getByText('Регулярность тренировок', { exact: true }).click()
