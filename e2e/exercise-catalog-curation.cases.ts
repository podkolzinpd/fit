...
for (const role of ['trainer', 'client'] as const) {
  test(`curated catalog keeps ${role} variants, aliases and forms usable in light/dark`, async ({ page }, testInfo) => {
    ...
    const profile = role === 'client' ? '/me/settings' : '/profile/settings'
    for (const dark of [false, true]) {
      ...
      await dialog.getByLabel('Поиск упражнения').fill('Подъём ног в упоре на локтях')
      await dialog.getByRole('button', { name: 'Проиграть технику: Подъём ног в упоре на локтях', exact: true }).click()
      await expect(dialog.getByRole('button', { name: 'Открыть технику: Подъём ног в упоре на локтях', exact: true })).toBeVisible()
      await expect(dialog.getByRole('heading', { name: 'Техника' })).toHaveCount(0)
      ...
      await dialog.getByLabel('Поиск упражнения').fill('Подъём камней Атласа')
      await expect(dialog.locator('[data-exercise-ref="fedb-atlas-stones"]')).toHaveCount(0)
      ...
      await dialog.getByRole('button', { name: 'Ноги', exact: true }).click()
      expect((await dialog.locator('.picker-list').boundingBox())?.height ?? 0).toBeGreaterThan(160)
      await page.screenshot({ path: testInfo.outputPath(`${role}-${dark ? 'dark' : 'light'}-filter-panel.png`) })
      await expect(dialog.getByRole('button', { name: 'Ноги', exact: true })).toHaveAttribute('aria-pressed', 'true')
